...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-32 axial low-entropy trigger`：在 commit `3b2d06e` 的 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增 `ANKLE_DECISION_TRAIN_AXIAL_ENTROPY_THRESHOLD`，并用 fresh adaptive `main-study` [runs/optuna_main_autoloop/iter_0012_20260423_090521](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0012_20260423_090521) 验证 hard low-entropy trigger 是否比 `top1-top2 margin` 更准确地捕捉真实的 axial starvation case。 |
| 上次结果 | **discard**。best completed trial `4` 为 `val_acc=0.9255319148936170`, `val_auc=0.9777272727272728`, `val_f1=0.9230769230769231`；`6/6` completed trial 形成 `0.9043 / 0.9149 / 0.9255` 三档，说明 entropy detector 确实比 margin family 更健康，但仍没有任何 trial 回到 retained `DFR-25/26` 的 `0.9362` ceiling，更没有新的 learned full-fusion 明确高于 matched `equal-weight` keep。 |
| 下一步 | 保持 `256x8 ResNeXt decision + L3-no-mixer + DFR-25` 主线不变，也**不要**回到 `top1-top2 margin` family。更直接的下一步应改测 **continuous low-entropy scaling**，仍保持 `DFR-25` + `floor=0.05` 不变，只把 hard entropy trigger 改成按 low-entropy 程度连续缩放 rescue，验证 entropy detector 能否在保留精度的同时覆盖 near-collapse 样本并重新守住 seed42 winning corner。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 7（`DFR-26 seed123`、`DFR-27 conditional axial-dominance floor`、`DFR-28 continuous axial-dominance-scaled rescue`、`DFR-29 reduced continuous rescue scale`、`DFR-30 axial top1-top2 margin trigger`、`DFR-31 continuous top1-top2 margin-scaled rescue` 与本轮 `DFR-32 axial low-entropy trigger` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。） |
