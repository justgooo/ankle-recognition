...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-78 same-class probability-advantage candidate gate seed42 candidates`：从 DFR-25 anchor 与 DFR-76 class-consensus candidate gate 出发，只新增 same-class probability-advantage guard，seed42 main-study 三候选 `prob_advantage=0.02/0.05/0.08`，其余 candidate 条件保持 `residual_limit=0.50`, `margin_gap=0.5`, `consensus_margin=0.35`, `consensus_window=0.20`, `prob_window=0.05`。 |
| 上次结果 | 三个 trial 完成，best trial0 `prob_advantage=0.02` 得到 `val_acc=0.9042553191489362`，`val_auc=0.9713636363636364`，`val_f1=0.8888888888888888` → discard，低于 DFR-25 seed42 和 matched equal-weight seed42。best fusion telemetry mean weights `0.4087/0.5371/0.0542`，top-weight count/rate `39/0.4149, 55/0.5851, 0/0.0000`；更强阈值进一步推向 coronal collapse（trial1 `14/80/0`，trial2 `1/93/0`）。 |
| 下一步 | 不 promotion DFR-78，也不要继续扫 probability/margin/consensus 阈值。若外层 loop 继续，应优先做样本级 trigger audit，或引入显式 supervised/prototype alignment signal，让 candidate/hierarchical selection 的 non-axial 触发对应真实 right-view evidence；继续避免 broad view-role amplitude、plain floor、direct teacher blend、scalar temperature、blind prior-debias 和 candidate-threshold sweep。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 53（自 `DFR-26 seed123` 起至 `DFR-78 same-class probability-advantage candidate gate seed42 candidates` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77/78 follow-up 离线分析不改变 discard 计数。） |
