...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-86 non-axial-only gate view-correctness auxiliary loss`：main-study；在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增 default-off `ANKLE_DECISION_GATE_VIEW_CORRECTNESS_AUX_NONAXIAL_ONLY`，只把 gate view-correctness aux 的 fusion-weight scaling 作用到 coronal/sagittal，fused logits 不变；seed42 三候选 `aux_weight=0.005/0.01/0.02`。 |
| 上次结果 | commit `4c4b9cf`，fresh main-study `iter_0013_20260512_074837` 完成 3/3 trials：trial0 `val_acc/val_auc/val_f1=0.925532/0.975455/0.923077`，trial1 `0.904255/0.978636/0.898876`，trial2 `0.925532/0.977273/0.917647`。best trial2 只打平 matched equal-weight seed42 且低于 DFR-25 seed42，因此 discard；权重验证 mean axial/coronal/sagittal=`0.5663/0.1515/0.2822`，top-weight=`60/0/34`，true-margin=`87/4/3`，`hit_rate.true_margin=0.5957`，说明 non-axial-only aux 能打破 axial top collapse，但过度迁移到 sagittal，未形成 right-view routing 增益。 |
| 下一步 | 不再继续扫 aux weight / base scale / weight scale。若外层 loop 继续，优先做 error-aware supervised pairwise ranking / selector target 的离线验证或保守训练设计：必须显式保护 DFR-25-correct 样本，并只在 oracle-correct non-axial error / true-margin 有证据样本上推动非 axial 贡献；继续避免 broad view-role amplitude、plain floor、direct teacher blend、scalar temperature、blind prior-debias 和 candidate-threshold sweep。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 61（自 `DFR-26 seed123` 起至 `DFR-86 non-axial-only gate view-correctness auxiliary loss` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77/78/79/80/81/82/83/84/85 follow-up 离线分析不改变 discard 计数语义，但本轮 ledger 仍按 discard 记录。） |
