...
- **Agent 状态（2026-05-28 SGT）**：当前主线为 feature-fusion exploration；active Slurm job=`none`。Last completed=`FF-XVIEW-ROLE-EMBEDDING-3SEED-MAIN`, lane=`main-study`, result=`discard`, 3-seed mean `0.932624/0.976364/0.928035`; role embeddings trained with nonzero view-role norm ratios but did not improve mean `val_acc` over the gated-head anchor. Current retained feature-fusion evidence remains full gated-head (`fe42d90`, mean `0.939716/0.975909/0.934415`). Next=`FF-PAIRWISE-FIXED-BROKEN-PATTERN-AUDIT` read-only validation pattern audit before further feature-fusion structural edits.
...
| 字段 | 值 |
|------|-----|
| 上次实验 | `FF-PAIRWISE-CASE-STATS`：完成 gated-head vs minimal baseline 的 validation paired fixed/broken 与 bootstrap/CI 只读统计。 |
| 上次结果 | commit `cd5c686` 已生成 `autoresearch_logs/ff_pairwise_case_stats.{json,md}`；checkpoint re-eval gated-head vs minimal mean delta `+0.021276/+0.003334/+0.023371`，canonical summary delta `+0.024823/+0.003788/+0.027573`；paired rows `282`，fixed `12`，broken `6`，net `+6`，per-seed net `+2/+3/+1`，McNemar p `0.237885`，patient-cluster bootstrap accuracy delta CI `[-0.010638, 0.053191]`。五轮 structure sweep 仍全部 discard，current retained feature-fusion model 仍为 full gated-head `fe42d90` mean `0.939716/0.975909/0.934415`。 |
