...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-56 pairwise reliability gate multiseed formal`：从 DFR-25 fork，只在 gate/reliability path 上加入低容量 `PairwiseReliabilityGate`，对每个 anchor view 聚合它与另外两个视角的 pair relation 后输出 bounded residual confidence logit；formal seeds `42/123/456` 在 Slurm job `482374` 上并行完成，权重 telemetry 已补齐。 |
| 上次结果 | **discard**。3-seed mean `val_acc=0.9007092198581560`, `val_auc=0.9683333333333334`, `val_f1=0.8846146373686154`，主指标低于 DFR-25 mean `0.9397163120567376 / 0.9677272727272728 / 0.9358934169278997`，也低于 matched equal-weight mean `0.9219858156028368 / 0.9606060606060606 / 0.9112221100424511` 的 `val_acc / val_f1`。fusion telemetry 显示 seed42/456 仍 axial top-weight `94/94`，seed123 改为 coronal top-weight `94/94`，说明 pairwise context 只把 axial lock-in 换成 seed-specific single-plane lock-in，没有学到稳定 ranking。 |
| 下一步 | 继续留在 DFR-25 decision-fusion collapse repair 主线，但第二轮应避开无监督 pair/Transformer gate-context 容量。更直接的下一步是 `DFR-57 view-id/anatomical-role calibrated reliability scoring` 或 GVCNN-style discrimination gate：显式建模切面身份和判别性，让 gate 学到低容量、可解释的视角角色，而不是继续围绕 axial 写规则。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 31（自 `DFR-26 seed123` 起至 `DFR-56 pairwise reliability gate multiseed formal` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。虽然计数已超过 5，但用户已明确要求本次继续完成 3 轮 manual autoresearch，且当前仍有直接服务于 `innovation.md` 和单视图独占修复的 view-id / discrimination gate 思路，因此继续执行而不暂停询问。） |
