...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-80 supervised prototype auxiliary candidate gate seed42 candidates`：从 DFR-25 anchor 与 DFR-76/79 class-consensus candidate gate 出发，只新增 supervised prototype auxiliary CE，让 candidate gate 的 class-prototype logits 受标签监督；seed42 main-study 三候选 `candidate_view_prototype_aux_weight=0.02/0.05/0.10`，其余 candidate 条件保持 `residual_limit=0.50`, `margin_gap=0.5`, `consensus_margin=0.35`, `consensus_window=0.20`, `prototype_window=0.05`。 |
| 上次结果 | 三个 trial 完成，`val_acc` 为 `0.8936170212765957 / 0.8936170212765957 / 0.9148936170212766`；best trial2 `aux_weight=0.10`，`val_auc=0.9472727272727273`，`val_f1=0.9000000000000000` → discard，低于 DFR-25 seed42 和 matched equal-weight seed42。best fusion telemetry mean weights `0.9176/0.0320/0.0504`，top-weight count/rate `94/1.0000, 0/0.0000, 0/0.0000`；true-margin 仍有 `83/4/7`，说明 supervised prototype aux 仍未让 non-axial true-evidence 样本进入 top routing。 |
| 下一步 | 不 promotion DFR-80，也不要继续扫 prototype aux weight/temperature、prototype advantage/window 或 candidate probability/margin/consensus 阈值。若外层 loop 继续，应优先做样本级 trigger audit，或换成更直接的 pairwise reliability / lightweight view-graph evidence agreement 机制；继续避免 broad view-role amplitude、plain floor、direct teacher blend、scalar temperature、blind prior-debias 和 candidate-threshold sweep。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 55（自 `DFR-26 seed123` 起至 `DFR-80 supervised prototype auxiliary candidate gate seed42 candidates` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77/78/79/80 follow-up 离线分析不改变 discard 计数。） |
