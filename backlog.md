...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-29 reduced continuous rescue scale`：在 commit `72f0bf3` 的 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增 `ANKLE_DECISION_TRAIN_NONDOMINANT_RESCUE_SCALE`，把 `DFR-28` 的 continuous rescue 最大混合强度从实现里解耦出来，并用 fresh adaptive `main-study` [runs/optuna_main_autoloop/iter_0009_20260423_065356](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0009_20260423_065356) 验证 `rescue_scale=0.5` 的更轻干预是否能守住 seed42 winning corner。 |
| 上次结果 | **discard**。best completed trial `0` 只有 `val_acc=0.9148936170212766`, `val_auc=0.9836363636363636`, `val_f1=0.9090909090909091`；更关键的是 `6/6` completed trial 全部停在同一 `0.9149` accuracy 平台，连 `DFR-25/26` 的 winning corner 也一起掉下来。这让本轮结论很明确：把 continuous rescue cap 减半不是温和修正，而是会系统性削弱整条 repair，甚至使 learned full-fusion 再次低于 matched `equal-weight` seed42。 |
| 下一步 | 保持 `256x8 ResNeXt decision + L3-no-mixer + DFR-25` 主线不变，但**停止继续压低 rescue cap / floor**。更直接的下一步应转向 **更精准的 full-strength trigger**：保持 `0.05` rescue 量不变，只改触发条件本身，例如测试 `gate entropy` 或 `top1-top2 margin` 型条件，让 full-strength 弱视角梯度只落在真正的 axial collapse 样本上；在 seed42 再次明确超过 retained keep 之前，不要回到 `seed=123` exact confirmation。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 4（`DFR-26 seed123`、`DFR-27 conditional axial-dominance floor`、`DFR-28 continuous axial-dominance-scaled rescue` 与本轮 `DFR-29 reduced continuous rescue scale` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。） |
