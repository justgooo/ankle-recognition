...
- [ ] **FF-ATTNPOOL-SLICE-COUNT-3SEED-FORMAL**：人类要求从 slice pooling / slice count 方向提高当前 feature-fusion gated-head anchor；使用 `scripts/autoresearch_main.py` + `configs/optuna_main_search_ff_attnpool_slices_3seed.yaml` 跑 4 组固定 trial：`use_attention_pooling=true` 且 `num_slices_per_view=8/12/16/20`，每组 seeds `42/123/456`，共 12 次 formal 训练；保持 `ResNeXt 256x8 base geometry family`, `trim_edge_slices=2`, `freeze_layers=3`, `lr=1e-4`, `wd=1e-4`, `dropout=0.25`, `clip=1.0`。设计思路：把每视角 slice mean pooling 改为可学习 AttentionPooling，并比较更多切片是否给 pooling scorer 提供更完整局部证据。预计改进效果：如果关键病灶只落在少数切片或 8-slice 均匀采样稀释了局部特征，attention pooling 应在同等 feature-fusion head 下提高 view token 质量，增加 3-seed mean `val_acc`；若更多切片引入噪声或训练漂移，`8/12/16/20` 的 3-seed mean 会显示上限。执行状态：按人类更新允许非 `--exclusive`，旧 exclusive pending job `517424` 已取消；commit `5a14a31` 后共有两个 active shared-node worker 写同一 study：job `517698`（`NA100q/node01`, requested `gres/gpu:6`, actual idle subset `6,7`, `WORKERS=2`, running trial `0/1`）和 job `517764`（`PA100q/node05`, requested `gres/gpu:1`, actual idle subset `1`, `WORKERS=1`, running trial `2`）；后处理 job `517773` 依赖 `afterok:517698:517764`，完成后汇总 `results.tsv`/`backlog.md`；study_root=`runs/optuna_main_ff_attnpool_slices_3seed`。
...
- **Agent 状态（2026-05-26 SGT）**：当前主线已切到 feature-fusion exploration；last completed=`PAPER-FEATURE-FUSION-MODULE-ABLATIONS-3SEED-FORMAL` (`c709f7c`, Slurm job `516767`)；last launched=`FF-ATTNPOOL-SLICE-COUNT-3SEED-FORMAL` (`5a14a31`, running jobs `517698` + `517764`, 3 active idle-GPU workers across `NA100q/node01` and `PA100q/node05`, postprocess dependency job `517773`); last retained feature-fusion evidence remains full gated-head (`fe42d90`, mean `0.939716/0.975909/0.934415`)；module ablations show all three submodules contribute, with largest drop from removing GLU head.
