...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-49 gate view-prior debias`：从 `DFR-25` fork，开启 `ANKLE_DECISION_ENABLE_GATE_VIEW_PRIOR_DEBIAS=1`，在 reliability logits softmax 前减去 running centered per-view confidence-logit prior；fresh single-GPU adaptive `main-study` [runs/optuna_main_autoloop/iter_0003_20260510_205859](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0003_20260510_205859) 已完成，权重 telemetry 已补齐。 |
| 上次结果 | **discard**。best `val_acc=0.8723404255319149`, `val_auc=0.9563636363636364`, `val_f1=0.8461538461538461`，低于 matched equal-weight seed42 与 DFR-25/26 seed42。fusion telemetry 为 mean weight `0.4140 / 0.3111 / 0.2749`，top-weight count/rate `axial=43/0.4574`, `coronal=25/0.2660`, `sagittal=26/0.2766`；它打破了 axial lock-in，但 full-strength prior subtraction 过度削弱真实 axial prior，routing 迁移没有带来 accuracy-positive full-fusion。 |
| 下一步 | 继续留在 DFR-25 decision-fusion collapse repair 主线，但不要再做 full-strength view-prior subtraction。更直接的下一步是把 DFR-49 改成 partial / conditional debias（例如先测低强度 `strength<1`，或只在 non-axial residual evidence 足够接近 axial 时启用），目标是在保住 DFR-25 axial 主导样本的同时迁移少量 non-axial true-margin 样本。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 24（自 `DFR-26 seed123` 起至 `DFR-49 gate view-prior debias` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。虽然计数已超过 5，但当前仍有直接服务于人类新增“权重坍塌”主线的最小 gate 内部结构思路，因此按规则继续执行而不暂停询问。） |
