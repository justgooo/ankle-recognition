...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-124 duplicate/leakage metric sensitivity`：只读计算 val-val duplicate、train-val duplicated validation patients 与 combined clean policy 对 DFR-25/DFR-116 validation 指标的影响。 |
| 上次结果 | commit `a446411`；报告 [autoresearch_logs/dfr124_duplicate_leakage_metric_sensitivity.json](/dataset/HH/ankle-ct/autoresearch_logs/dfr124_duplicate_leakage_metric_sensitivity.json)。combined clean sensitivity 下 DFR-25 为 `0.948718/0.979024/0.944052`，DFR-116 为 `0.959707/0.982439/0.955819`，DFR-116 delta 仍为 `+0.010989/+0.003415/+0.011767`；train-val duplicate positives 在两分支三 seed 都预测正确，val-val duplicate 仍是三 seed repeated FN。 |
| 下一步 | DFR-125：做 combined-clean false-case/frontier audit，只读地在剔除 train-val duplicated val patients、val-val duplicate keep-first 的 policy 下重新列出 DFR-116 remaining errors 与 no-harm posthoc frontier；不修改数据文件、不使用 test 指标。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 0（DFR-124 diagnostic sensitivity keep；当前最优训练 checkpoint 仍是 DFR-25 3-seed formal mean，当前最优 posthoc calibration branch 仍是 DFR-116 strict combo。） |
