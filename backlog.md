...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-97 sagittal-normal rescue aux`：从 DFR-25 fork，新增默认关闭的 sagittal-normal pair auxiliary，只在 detached axial abnormal / sagittal normal 样本上用 learned axial+sagittal pair weights 做训练期 CE。 |
| 上次结果 | commit `2ac9151`，fresh `main-study` [runs/optuna_main_autoloop/iter_0002_20260512_203844](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0002_20260512_203844) 完成 3 个 seed42 候选；best trial1 `val_acc/val_auc/val_f1=0.925532/0.974091/0.921348`，只打平 matched equal-weight seed42，低于 DFR-25 seed42。fusion telemetry mean axial/coronal/sagittal=`0.7116/0.1860/0.1024`，top-weight=`81/13/0`，true-margin=`84/3/7`，说明没有释放 sagittal，反而产生 coronal side-effect，本轮 discard。 |
| 下一步 | 停止简单 `sagittal_normal_rescue_aux_weight` 调参；若继续主线，优先做 calibration-only / post-training gate-bias audit，或设计显式 view-index constrained 的 sagittal logit correction，目标是只校准 DFR-96 tiny sagittal target，避免 coronal 被动释放。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 72（自 `DFR-26 seed123` 起至 `DFR-97 sagittal-normal rescue aux` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77/78/79/80/81/82/83/84/85/96 follow-up 离线分析不改变训练失败计数语义，但本轮 ledger 仍按 discard 记录。） |
