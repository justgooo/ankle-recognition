...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-81 pairwise agreement candidate gate seed42 candidates`：从 DFR-25 anchor 与 DFR-76 class-consensus candidate gate 出发，只新增 pairwise non-axial evidence-agreement guard；coronal candidate 需 sagittal 同方向 signed-margin 支持，sagittal candidate 需 coronal 支持，seed42 main-study 三候选 `nonaxial_pair_consensus_margin=0.10/0.20/0.30`，其余 candidate 条件保持 `residual_limit=0.50`, `margin_gap=0.5`, `consensus_margin=0.35`, `consensus_window=0.20`, `nonaxial_pair_consensus_window=0.20`。 |
| 上次结果 | 三个 trial 完成，`val_acc` 为 `0.8936170212765957 / 0.9148936170212766 / 0.9042553191489362`；best trial1 `margin=0.20`，`val_auc=0.9427272727272727`，`val_f1=0.9024390243902439` → discard，低于 DFR-25 seed42 和 matched equal-weight seed42。best fusion telemetry mean weights `0.7997/0.1571/0.0432`，top-weight count/rate `88/0.9362, 6/0.0638, 0/0.0000`；true-margin `78/9/7`，说明 pairwise guard 释放了少量 coronal top-routing，但仍没有让 sagittal true-evidence 样本进入 top routing，也没有转成 accuracy gain。 |
| 下一步 | 不 promotion DFR-81，也不要继续扫 pairwise margin/window 或回到 candidate-threshold 微调。若外层 loop 继续，应优先做样本级 trigger audit / offline threshold study，定位 DFR-25 错误样本中哪些 coronal/sagittal true-margin 信号能真实修正决策，再决定是否做 lightweight view-graph / hierarchical selection；继续避免 broad view-role amplitude、plain floor、direct teacher blend、scalar temperature、blind prior-debias 和 candidate-threshold sweep。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 56（自 `DFR-26 seed123` 起至 `DFR-81 pairwise agreement candidate gate seed42 candidates` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77/78/79/80/81 follow-up 离线分析不改变 discard 计数。） |
