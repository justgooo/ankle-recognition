...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-88 disagreement-gated non-axial gate view-correctness aux`：main-study；只新增 config search，保持 DFR-25 anchor，并同时启用 `ANKLE_DECISION_GATE_VIEW_CORRECTNESS_AUX_REQUIRE_DISAGREEMENT=1` 与 `ANKLE_DECISION_GATE_VIEW_CORRECTNESS_AUX_NONAXIAL_ONLY=1`；seed42 三候选 aux weight `0.005/0.01/0.02`。 |
| 上次结果 | commit `020239d`，fresh main-study `iter_0015_20260512_083335` 完成 3/3 trials：trial0 `val_acc/val_auc/val_f1=0.914894/0.978182/0.909091`，trial1 `0.914894/0.981364/0.911111`，trial2 `0.925532/0.962273/0.919540`。best trial2 只打平 matched equal-weight seed42，低于 DFR-25 seed42，因此 discard；权重验证 mean axial/coronal/sagittal=`0.6735/0.1404/0.1861`，top-weight=`81/0/13`，true-margin=`84/4/6`，`hit_rate.true_margin=0.7553`，说明 disagreement + non-axial aux 释放了 sagittal routing，但没有把 coronal/sagittal 迁移对齐到正确样本。 |
| 下一步 | 关闭 DFR-83/86/88 simple gate-correctness aux 组合，不再扫 aux weight/base scale/disagreement/axial-protection阈值。若外层 loop 继续，应先设计能通过现有 `train.py` aux hook 表达的 error-aware selector/ranking objective，或做 analysis-only 可行性审计；继续避免 broad view-role amplitude、plain floor、direct teacher blend、scalar temperature、blind prior-debias 和 candidate-threshold sweep。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 63（自 `DFR-26 seed123` 起至 `DFR-88 disagreement-gated non-axial gate view-correctness aux` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77/78/79/80/81/82/83/84/85 follow-up 离线分析不改变 discard 计数语义，但本轮 ledger 仍按 discard 记录。） |
