...
| 字段 | 值 |
|------|-----|
| 上次实验 | CVT-MAN-14（winner-template stability probe；`seed=456`, `weight_decay=9e-4`; commit `5c45a03`） |
| 上次结果 | discard（`val_acc=0.8404255319148937`, `val_auc=0.8763636363636363`, `val_f1=0.810126582278481`, `peak_vram=14.4 GiB`。较 current proxy winner `0.8829787234042553` 低 `0.0425531914893616`；与 `CVT-MAN-10` 的 exact `seed=456` rerun accuracy 持平，但 AUC 回升 `0.0045454545454545`；较 `CVT-MAN-13` 的 `8.75e-4` 点 accuracy 回升 `0.0106382978723405`，但 AUC 下降 `0.0227272727272727`。说明 `9e-4` 是当前 `seed=456` / upper-side `weight_decay` 细化里首个把 accuracy 拉回 baseline-level 的点，也保留了相对 `1e-3` 的少量排序收益，但整体仍明显低于 current proxy winner，且继续在 `seed=456` 上做更细 L2 插值的边际信息量已有限。） |
| 下一步 | 结束 `seed=456` 的 `weight_decay` 细插轴；若继续做最高优先级未完成实验，应转测 `seed=123`, `weight_decay=9e-4` 的 winner-template stability probe，验证这次在 `seed=456` 上出现的 baseline-level accuracy 回补是否能跨 alternate seed 复现。若该点在 `seed=123` 上也无法逼近 current proxy winner，应停止 winner-template 的 alternate-seed L2 rescue，改回新的结构或训练策略方向。 |
| 连续 discard 计数 | 14 |
