...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-27 conditional axial-dominance floor`：在 commit `1176eb2` 的 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增 `ANKLE_DECISION_TRAIN_AXIAL_DOMINANCE_THRESHOLD`，并用 fresh adaptive `main-study` [runs/optuna_main_autoloop/iter_0007_20260423_051629](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0007_20260423_051629) 验证 `ANKLE_DISABLE_FUSION_CROSS_VIEW_MIXER=1` + `ANKLE_DECISION_TRAIN_DOMINANT_GATE_DROPOUT_PROB=0.25` + `ANKLE_DECISION_TRAIN_NONDOMINANT_WEIGHT_FLOOR=0.05` + `ANKLE_DECISION_TRAIN_AXIAL_DOMINANCE_THRESHOLD=0.8` 是否能只在 axial lock-in 样本上触发 weak-view rescue。 |
| 上次结果 | **discard**。best completed trial `3` 达到 `val_acc=0.9361702127659575`, `val_auc=0.9659090909090909`, `val_f1=0.9318181818181818`，仍高于 matched `equal-weight` seed42 `0.9255319148936170 / 0.9486363636363637 / 0.9176470588235294`，但只与 retained `DFR-25/26` seed42 keep 在 `val_acc` 上打平，且 `val_auc` 明显低于 `DFR-26` 的 `0.9800000000000000` 与 `DFR-25` 的 `0.9786363636363636`。同时，旧 winning corner 在本轮只得到 `0.9255319148936170 val_acc`，说明 hard conditional trigger 没有保住已知最好角点。 |
| 下一步 | 保持 `256x8 ResNeXt decision + L3-no-mixer + DFR-25` 主线不变，但**不要把这条 hard-threshold conditional floor 直接扩到 multiseed**。更合理的下一步仍是 training-only routing repair，不过应把触发改成 **continuous concentration-scaled rescue**（例如按 axial dominance excess 或低 entropy 平滑调节注入 mass），先在 `seed=42` 验证能否同时守住 `0.9362 val_acc` 与 `DFR-26` 的 AUC 收益，再决定是否回到 `seed=123` exact confirmation。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 2（`DFR-26 seed123` 与本轮 `DFR-27 conditional axial-dominance floor` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。） |
