...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-55 gate transformer context multiseed formal`：从 DFR-25 fork，只在 gate/reliability path 上加入 1-layer/4-head/128-dim `GateTransformerContextualizer`，输入三视角 centered relative features，zero-init residual projection；formal seeds `42/123/456` 在 Slurm job `482197` 上顺序完成，权重 telemetry 已补齐。 |
| 上次结果 | **discard**。3-seed mean `val_acc=0.8936170212765958`, `val_auc=0.9539393939393940`, `val_f1=0.8805292205745978`，低于 DFR-25 mean `0.9397163120567376 / 0.9677272727272728 / 0.9358934169278997`，也低于 matched equal-weight mean `0.9219858156028368 / 0.9606060606060606 / 0.9112221100424511`。fusion telemetry 显示 seed42/456 过度迁到 sagittal 或混合 routing，而 seed123 重新 hard axial lock-in；Transformer context 改变了权重形状，但没有学到可靠 ranking。 |
| 下一步 | 继续留在 DFR-25 decision-fusion collapse repair 主线，但不要再增加无监督 gate-context 容量。更直接的下一步是 `DFR-56 teacher/correctness-aligned gate ranking supervision` 或离线 DFR-25 disagreement telemetry：先找出 non-axial true-margin 样本的可靠触发条件，再做低容量、有上限的 learned correction。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 30（自 `DFR-26 seed123` 起至 `DFR-55 gate transformer context multiseed formal` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。虽然计数已超过 5，但当前仍有直接服务于人类新增“权重坍塌”主线的 gate-ranking 监督/telemetry 思路，因此按规则继续执行而不暂停询问。） |
