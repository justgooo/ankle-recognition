...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-48 relative confidence features`：从 `DFR-25` fork，开启 `ANKLE_DECISION_USE_RELATIVE_CONFIDENCE_FEATURES=1`，把 reliability scorer / calibrator 的主输入改成 sample-wise centered gating features；fresh single-GPU adaptive `main-study` [runs/optuna_main_autoloop/iter_0002_20260510_202918](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0002_20260510_202918) 已完成，权重 telemetry 已补齐。 |
| 上次结果 | **discard**。best `val_acc=0.9148936170212766`, `val_auc=0.9818181818181818`, `val_f1=0.9111111111111111`，低于 matched equal-weight seed42 与 DFR-25/26 seed42。fusion telemetry 为 mean weight `0.9173 / 0.0341 / 0.0486`，top-weight 仍 `axial=94/94`；虽然 `top_true_margin` 有 `sagittal=11`，gate 没有一次迁移到 non-axial，说明 centered confidence features 只降低平均 axial mass，没有改变 routing ordering。 |
| 下一步 | 继续留在 DFR-25 decision-fusion collapse repair 主线，但不要再做 feature-space relative residual 或 centered feature main-path 替换。更直接的下一步应测试 raw confidence per-view de-bias / view-prior removal，或极低容量 contrastive rank correction，使固定 axial logit offset 不能继续主导 softmax ordering。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 23（自 `DFR-26 seed123` 起至 `DFR-48 relative confidence features` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。虽然计数已超过 5，但当前仍有直接服务于人类新增“权重坍塌”主线的最小 gate 内部结构思路，因此按规则继续执行而不暂停询问。） |
