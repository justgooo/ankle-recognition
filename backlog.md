...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-82 DFR-25 non-axial trigger audit`：analysis-only，只新增 [scripts/analyze_dfr25_trigger_audit.py](/dataset/HH/ankle-ct/scripts/analyze_dfr25_trigger_audit.py)，读取 DFR-25 三 seed 以及 DFR-76/81 comparator 的既有 `fusion_weight_analysis.json`，审计 non-axial oracle 修复空间和 label-free trigger 的 helpful/harmful 覆盖。 |
| 上次结果 | commit `d52919f`，不训练新 checkpoint；引用 DFR-25 3-seed reference `0.9397163120567376/0.9677272727272728/0.9358934169278997`。DFR-25 三 seed 权重：seed42 mean `0.8886/0.0791/0.0324`, top `94/0/0`；seed123 mean `0.5253/0.1159/0.3588`, top `49/0/45`；seed456 mean `0.8701/0.0658/0.0640`, top `94/0/0`。三 seed 聚合 DFR-25 错样本 `17` 个，`top_true_margin axial/coronal/sagittal=1/6/10`；oracle view-correct trigger 可修复 `11/17` 且 `harm=0`，但 label-free triggers 均失败：pred-margin `0/4` helpful/harmful，class-consensus `0/1`，pair-consensus `0/1`，nonaxial-abnormal-over-axial `2/39` → discard / analysis-only negative。 |
...
| 连续 discard 计数 | 57（自 `DFR-26 seed123` 起至 `DFR-82 DFR-25 non-axial trigger audit` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77/78/79/80/81/82 follow-up 离线分析不改变 discard 计数语义，但本轮 ledger 仍按 discard 记录。） |
