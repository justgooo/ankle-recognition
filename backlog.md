...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-79 prototype-aligned candidate gate seed42 candidates`：从 DFR-25 anchor 与 DFR-76 class-consensus candidate gate 出发，只新增 learned class-prototype alignment guard，seed42 main-study 三候选 `prototype_advantage=0.00/0.05/0.10`，其余 candidate 条件保持 `residual_limit=0.50`, `margin_gap=0.5`, `consensus_margin=0.35`, `consensus_window=0.20`, `prototype_window=0.05`。 |
| 上次结果 | 三个 trial 完成，`val_acc` 全部 `0.9042553191489362`；best by AUC 为 trial2 `prototype_advantage=0.10`，`val_auc=0.9650000000000000`，`val_f1=0.8965517241379310` → discard，低于 DFR-25 seed42 和 matched equal-weight seed42。best fusion telemetry mean weights `0.8861/0.0898/0.0240`，top-weight count/rate `93/0.9894, 1/0.0106, 0/0.0000`；true-margin 仍有 `77/12/5`，说明 prototype guard 过滤了 coronal over-routing 但恢复 axial dependence，没有产生有效 non-axial contribution。 |
| 下一步 | 不 promotion DFR-79，也不要继续扫 prototype advantage/window 或 candidate probability/margin/consensus 阈值。若外层 loop 继续，应优先做样本级 trigger audit，或把 prototype alignment 做成显式 supervised/center-style auxiliary signal 后再用于 routing；继续避免 broad view-role amplitude、plain floor、direct teacher blend、scalar temperature、blind prior-debias 和 candidate-threshold sweep。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 54（自 `DFR-26 seed123` 起至 `DFR-79 prototype-aligned candidate gate seed42 candidates` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77/78/79 follow-up 离线分析不改变 discard 计数。） |
