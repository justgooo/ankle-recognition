...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-77 detached class-consensus candidate-view reliability gate seed42 candidates`：从 DFR-25 anchor 出发，在 DFR-76 class-consensus candidate gate 上只新增 `ANKLE_DECISION_CANDIDATE_VIEW_GATE_DETACH_FEATURES=1`，seed42 main-study 三候选 `residual_limit=0.35/0.50/0.65`，其余 candidate 条件保持 `margin_gap=0.5`, `consensus_margin=0.35`, `consensus_window=0.20`。 |
| 上次结果 | 三个 trial 全部 `val_acc=0.9042553191489362`，`val_auc=0.9786363636363636`，`val_f1=0.8888888888888888` → discard，低于 DFR-25 seed42 和 matched equal-weight seed42。best trial0 fusion telemetry 为 mean weights `0.8991/0.0521/0.0487`，top-weight count/rate `94/1.0000, 0/0.0000, 0/0.0000`；detach 后 DFR-76 的 non-axial routing 迁移消失，恢复 axial single-view dependence。 |
| 下一步 | 不 promotion DFR-77，也不要继续扫 `residual_limit` 或只在 detach/不 detach 间切换。若外层 loop 继续，应先做样本级 trigger audit，或引入显式 supervised/prototype alignment signal，让 candidate/hierarchical selection 的 non-axial 触发对应真实 right-view evidence；继续避免 broad view-role amplitude、plain floor、direct teacher blend、scalar temperature、blind prior-debias 和 candidate-threshold sweep。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 52（自 `DFR-26 seed123` 起至 `DFR-77 detached class-consensus candidate-view reliability gate seed42 candidates` 连续为 discard；DFR-57/60/61/62/63/64/65/66/67/68/69/70/71/72/73/74/75/76/77 follow-up 离线分析不改变 discard 计数。） |
