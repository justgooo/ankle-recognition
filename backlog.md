...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-53 midcap evidence-close prior debias`：从 `DFR-52` fork，只把 evidence-close conditional prior-debias `ANKLE_DECISION_GATE_VIEW_PRIOR_DEBIAS_MAX_STRENGTH` 从 `0.55` 提到 `0.75`；fresh single-GPU adaptive `main-study` [runs/optuna_main_autoloop/iter_0002_20260510_231151](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0002_20260510_231151) 已完成，权重 telemetry 已补齐。 |
| 上次结果 | **discard**。best `val_acc=0.9361702127659575`, `val_auc=0.9568181818181818`, `val_f1=0.9302325581395349`，高于 matched equal-weight seed42 但只打平 retained DFR-25/26 seed42 accuracy，且 AUC 明显更低。fusion telemetry 为 mean weight `0.4884 / 0.2116 / 0.3000`，top-weight count/rate `axial=54/0.5745`, `coronal=12/0.1277`, `sagittal=28/0.2979`；它把 routing 推到目标中间区间，但 `top_weight_hit_rate.true_margin=0.5851`，没有带来优于 retained learned full-fusion 的 ranking quality。 |
| 下一步 | 继续留在 DFR-25 decision-fusion collapse repair 主线，但不要继续盲扫 `max_strength`。更直接的下一步是 `DFR-54 per-view capped or teacher-aligned conditional prior debias`：限制 non-axial 迁移只发生在更可靠的 evidence/teacher-aligned 样本上，目标是保留 DFR-53 的 non-axial routing 打开能力，同时恢复 DFR-25/26 级 AUC/accuracy tie-break。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 28（自 `DFR-26 seed123` 起至 `DFR-53 midcap evidence-close prior debias` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。虽然计数已超过 5，但当前仍有直接服务于人类新增“权重坍塌”主线的最小 gate 内部结构思路，因此按规则继续执行而不暂停询问。） |
