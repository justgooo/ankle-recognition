...
- [x] **FF-ATTNPOOL-SLICE-COUNT-3SEED-FORMAL**：人类要求从 slice pooling / slice count 方向提高当前 feature-fusion gated-head anchor；使用 `scripts/autoresearch_main.py` + `configs/optuna_main_search_ff_attnpool_slices_3seed.yaml` 跑 4 组固定 trial：`use_attention_pooling=true` 且 `num_slices_per_view=8/12/16/20`，每组 seeds `42/123/456`，共 12 次 formal 训练；保持 `trim_edge_slices=2`, `freeze_layers=3`, `lr=1e-4`, `wd=1e-4`, `dropout=0.25`, `clip=1.0`。设计思路：把每视角 slice mean pooling 改为可学习 AttentionPooling，并比较更多切片是否给 pooling scorer 提供更完整局部证据。预计改进效果：如果关键病灶只落在少数切片或 8-slice 均匀采样稀释了局部特征，attention pooling 应提高 view token 质量。实验实际结果：Slurm job `517698+517764` 完成；`S=8` mean `0.893617/0.960606/0.876527`; `S=12` mean `0.907801/0.946515/0.893737`; `S=16` mean `0.904255/0.963030/0.895339`; `S=20` mean `0.897163/0.950909/0.887205`；best `S=12` mean `0.907801/0.946515/0.893737`；best `S=12` remains below current gated-head anchor `0.939716/0.975909/0.934415` → **discard family as candidate**；report=`autoresearch_logs/ff_attnpool_slices_3seed_summary.json`。
- [x] **FF-STRUCT-01-GLU-HIDDEN384-3SEED-FORMAL**：人类要求在干净三卡节点上继续 feature-fusion 主线三种子五轮训练；本轮只把当前 gated-head anchor 的 GLU hidden width 从 `256` 增到 `384`，保留 `ResNeXt 256x8`, mean pooling, trim2, recalibration, light CrossViewAttention, freeze3, lr/wd/dropout/clip 与 seeds `42/123/456`。设计思路：扩大 fused-token GLU head 的中间宽度，检验当前 full gated-head 是否受融合头容量限制。预计改进效果：如果 fused-token gating 是主要收益源且容量不足，更宽 GLU 应提高跨视角特征组合能力；如果当前头部容量已足够，扩大宽度会增加方差或过拟合。实验实际结果：Slurm job `518423` 在 `V100q/node21` 三卡完成本组；seeds `42/123/456` achieved `0.893617/0.970000/0.880952`, `0.893617/0.940909/0.880952`, `0.936170/0.965455/0.930233`；3-seed mean `0.907801/0.958788/0.897379`, peak_vram max `2.17 GiB` → **discard**（低于当前 gated-head anchor `0.939716/0.975909/0.934415`）。
- [x] **FF-STRUCT-02-GLU-HIDDEN128-3SEED-FORMAL**：同一 Slurm job `518423` 的第二个结构轮次，只把当前 gated-head anchor 的 GLU hidden width 从 `256` 降到 `128`，其余 feature-fusion geometry/scalars/seeds 不变。设计思路：缩小 fused-token GLU head，检验 anchor 是否因融合头过宽而产生 seed 方差或过拟合。预计改进效果：如果当前头部容量过大，较窄 GLU 应改善小数据稳定性并提高三种子均值；如果容量不足，压缩会损失跨视角组合能力。实验实际结果：seeds `42/123/456` achieved `0.904255/0.968182/0.891566`, `0.957447/0.945000/0.952381`, `0.882979/0.965000/0.881720`；3-seed mean `0.914894/0.959394/0.908556`, peak_vram max `2.16 GiB` → **discard**（seed123 单点较高但均值低于 gated-head anchor）。
- [x] **FF-STRUCT-03-XVIEW-RESIDUAL00625-3SEED-FORMAL**：同一 Slurm job `518423` 的第三个结构轮次，只把 light CrossViewAttention mixer 的 residual scale 从 anchor `0.125` 降到 `0.0625`，其余 feature-fusion geometry/scalars/seeds 不变。设计思路：降低跨视角 token mixer 注入强度，检验当前 anchor 是否因 cross-view interaction 过强而破坏 per-view 表征稳定性。预计改进效果：如果 mixer 过强，减半 residual 应保留有用跨视角上下文同时降低噪声扩散；如果 anchor scale 正合适，减弱后应损失融合收益。实验实际结果：seeds `42/123/456` achieved `0.946809/0.978636/0.943820`, `0.925532/0.969545/0.915663`, `0.904255/0.971364/0.886076`；3-seed mean `0.925532/0.973182/0.915186`, peak_vram max `2.17 GiB` → **discard**（当前五轮最好但仍低于 gated-head anchor `0.939716/0.975909/0.934415`）。
- [x] **FF-STRUCT-04-XVIEW-RESIDUAL025-3SEED-FORMAL**：同一 Slurm job `518423` 的第四个结构轮次，只把 light CrossViewAttention mixer 的 residual scale 从 anchor `0.125` 提高到 `0.25`，其余 feature-fusion geometry/scalars/seeds 不变。设计思路：提高跨视角 token mixer 注入强度，检验当前 anchor 是否 under-share non-dominant view context。预计改进效果：如果当前 mixer 过弱，增强 residual 应帮助 fused representation 在 concatenation 前整合互补切面证据；如果 anchor 已平衡，则更强交互会放大噪声。实验实际结果：seeds `42/123/456` achieved `0.936170/0.967727/0.930233`, `0.925532/0.972273/0.921348`, `0.914894/0.982727/0.902439`；3-seed mean `0.925532/0.974242/0.918007`, peak_vram max `2.17 GiB` → **discard**（低于 gated-head anchor）。
- [x] **FF-STRUCT-05-XVIEW-DIM512-3SEED-FORMAL**：同一 Slurm job `518423` 的第五个结构轮次，只把 CrossViewAttention internal attention dimension 从 `256` 提高到 `512`，保留一层、四头、residual scale `0.125` 与其余 feature-fusion geometry/scalars/seeds 不变。设计思路：扩大 cross-view attention 的通道容量，检验 256-dim mixer 是否压缩了跨切面互补特征。预计改进效果：如果当前 mixer bottleneck 丢失有用通道细节，512-dim attention 应提高 feature-level interaction；如果不是瓶颈，会增加方差。实验实际结果：seeds `42/123/456` achieved `0.914894/0.971364/0.900000`, `0.946809/0.954091/0.939759`, `0.904255/0.952727/0.896552`；3-seed mean `0.921986/0.959394/0.912104`, peak_vram max `2.18 GiB` → **discard**（五轮结构 sweep 全部低于 gated-head anchor）。
- [x] **FF-PAIRWISE-CASE-STATS**：基于已完成 gated-head vs minimal baseline 3-seed validation telemetry，新增 [scripts/report_ff_pairwise_case_stats.py](/dataset/HH/ankle-ct/scripts/report_ff_pairwise_case_stats.py) 与 Slurm 入口 [scripts/slurm_ff_pairwise_case_stats.sbatch](/dataset/HH/ankle-ct/scripts/slurm_ff_pairwise_case_stats.sbatch)，生成 [autoresearch_logs/ff_pairwise_case_stats.json](/dataset/HH/ankle-ct/autoresearch_logs/ff_pairwise_case_stats.json) 和 `.md` 报告；不使用测试集指标。设计思路：用 paired seed-case transition 和 patient-cluster bootstrap 检查 full gated-head 相对 minimal concat-MLP 的增益是否来自稳定样本级修复，而不是只看三种子均值。预计改进效果：如果 recalibration + xview mixer + GLU head 的组合是真实增益，应看到 fixed_by_gated 多于 broken_by_gated，且三 seed 都是正向 net delta。实验实际结果：checkpoint re-eval 3-seed mean gated-head `0.936170/0.975455/0.930213` vs minimal `0.914894/0.972121/0.906842`，delta `+0.021276/+0.003334/+0.023371`；canonical training-summary delta 为 `+0.024823/+0.003788/+0.027573`。paired seed-case rows `282`，fixed_by_gated `12`，broken_by_gated `6`，net `+6`；per-seed net `+2/+3/+1`；McNemar exact p `0.237885`，patient-cluster bootstrap accuracy delta CI `[-0.010638, 0.053191]` → **keep as diagnostic evidence**（当前 retained feature-fusion model 仍为 full gated-head，不产生新 checkpoint）。
- [x] **FF-XVIEW-ROLE-EMBEDDING-3SEED-MAIN**：commit `44196ce`（model change introduced at `8dc96fe`），使用本轮 fresh study `runs/optuna_main_autoloop/iter_0001_20260528_004844`，通过 `study.env.ANKLE_FEATURE_ENABLE_VIEW_ROLE_EMBEDDING=1` 在 feature-fusion gated-head anchor 的 pooled per-view tokens 上加入零初始化 anatomical view-role embedding，只在 CrossViewAttention mixer 前提供 axial/coronal/sagittal 角色编码；其余 `ResNeXt 256x8`, mean pooling, trim2, recalibration, light CrossViewAttention, GLU head, freeze3, lr/wd/dropout/clip 与 seeds `42/123/456` 保持不变。设计思路：当前 light xview mixer 对 3 个 view tokens 没有显式视角角色输入；给 mixer 前 token 加可学习 view-role embedding，检验 cross-view attention 是否需要视角身份来学习非对称 axial/coronal/sagittal 上下文交换。预计改进效果：若 anchor 的 cross-view token interaction 受“视角角色不可分”限制，role token 应改善 fused representation 中各切面的角色可分性，使后续 GLU fused-token head 获得更稳定的多视角组合输入，并通过更少的视角混淆提高 `val_acc`；如果 anchor 已能从特征分布隐式识别视角，则收益应很小或增加 seed 方差。实验实际结果（含权重比例验证）：main-study seeds `42/123/456` achieved `0.946809/0.974091/0.941176`, `0.936170/0.979091/0.931818`, `0.914894/0.975909/0.911111`；3-seed mean `0.932624/0.976364/0.928035`, peak_vram max `2.17 GiB`。checkpoint 中 `feature_view_role_embedding` 确实非零学习，axial/coronal/sagittal norm ratios 约为 seed42 `0.289/0.374/0.338`, seed123 `0.290/0.375/0.334`, seed456 `0.293/0.378/0.330`；但 mean `val_acc` 低于 current gated-head anchor `0.939716/0.975909/0.934415`，仅 AUC 略高 → **discard**。
- [x] **FF-PAIRWISE-TOKEN-RESIDUAL-3SEED-MAIN**：commit `baa8347`，使用 fresh study `runs/optuna_main_autoloop/iter_0002_20260528_013823`，通过 `study.env.ANKLE_FEATURE_ENABLE_PAIRWISE_TOKEN_RESIDUAL=1` 在 feature-fusion gated-head anchor 的 CrossViewAttention mixer 后加入零初始化低容量 pairwise view-token residual；该 residual 用每个 view token 相对其他 view 的差值与乘积生成小幅 token 修正，其余 `ResNeXt 256x8`, mean pooling, trim2, recalibration, light CrossViewAttention, GLU head, freeze3, lr/wd/dropout/clip 与 seeds `42/123/456` 保持不变。设计思路：让融合头在拼接前直接看到两两视角一致/冲突线索，而不是只依赖 self-attention 后的三个独立 token。预计改进效果：如果当前 fused-token GLU head 缺少显式 pairwise cross-view context，pairwise residual 应改善 feature-level multi-view representation，给 GLU head 更稳定的组合输入并提高 `val_acc`；零初始化末层让训练从 anchor 行为开始，降低额外模块扰动。实验实际结果（含权重比例验证）：main-study seeds `42/123/456` achieved `0.925532/0.958182/0.919540`, `0.925532/0.956818/0.915663`, `0.914894/0.964545/0.906977`；3-seed mean `0.921986/0.959848/0.914060`, peak_vram max `2.18 GiB`。checkpoint 中 pairwise residual 已学习非零参数，final adapter weight norms seed42/123/456 为 `0.229/0.124/0.281`，pair_encoder weight norms 为 `0.188/0.014/0.290`；但 mean `val_acc` 低于 gated-head anchor `0.939716/0.975909/0.934415` → **discard**。
- [ ] **FF-PAIRWISE-FIXED-BROKEN-PATTERN-AUDIT**：基于 `FF-PAIRWISE-CASE-STATS` 的 fixed/broken seed-case 列表，继续做只读样本模式审计，聚焦 recurring fixed/broken patients、abnormal probability deltas、label balance 与已完成 ablation/structure variants 的一致性；目标是在不使用测试集、不训练新模型的前提下，判断下一轮 feature-fusion 改动应保护哪些 gated-head gains、避免哪些 broken cases。
- **Agent 状态（2026-05-28 SGT）**：当前主线为 feature-fusion exploration；active Slurm job=`none`。Last completed=`FF-PAIRWISE-TOKEN-RESIDUAL-3SEED-MAIN`, lane=`main-study`, result=`discard`, 3-seed mean `0.921986/0.959848/0.914060`; pairwise residual parameters trained nonzero but did not improve mean `val_acc` over the gated-head anchor. Current retained feature-fusion evidence remains full gated-head (`fe42d90`, mean `0.939716/0.975909/0.934415`). Next=`FF-PAIRWISE-FIXED-BROKEN-PATTERN-AUDIT` read-only validation pattern audit before further feature-fusion structural edits.
...
### DFR-88 disagreement-gated non-axial gate view-correctness aux（2026-05-12）

> **实验说明**
> - 本轮启动时发现 DFR-74 hard mismatch dominant-gate dropout formal confirmation 已在当前 ledger 中完成并关闭；为避免重复已完成实验，本轮继续从 DFR-25 anchor 做一个 seed42 三候选研究迭代。
> - 唯一离散机制是把既有 gate view-correctness aux 同时限制为 `ANKLE_DECISION_GATE_VIEW_CORRECTNESS_AUX_REQUIRE_DISAGREEMENT=1` 与 `ANKLE_DECISION_GATE_VIEW_CORRECTNESS_AUX_NONAXIAL_ONLY=1`，保持 ResNeXt 256x8、learned decision fusion、L3 no-mixer、dominant-gate dropout、winning scalar 不变；代码只新增 [configs/optuna_main_search_resnext_decision_256x8_dfr88_disagreement_nonaxial_gate_aux.yaml](/dataset/HH/ankle-ct/configs/optuna_main_search_resnext_decision_256x8_dfr88_disagreement_nonaxial_gate_aux.yaml)，不改 `train.py` / 数据 / backbone。
> - 设计思路：DFR-86 证明 non-axial-only aux 能释放 sagittal routing 但过度迁移；DFR-87 又证明 axial-margin protection 会过度保护并回到 `94/0/0` axial lock-in。DFR-88 因此只在三视角 detached prediction 出现 disagreement 的样本上让 coronal/sagittal 获得 aux scaling，all-agree 样本尽量保留 DFR-25 的 axial strong path。
> - 预计改进效果：相对 DFR-87 best 的 `top_weight=94/0/0` 和 DFR-86 best 的 `60/0/34`，预期 routing 落在中间的 axial-majority 区间，约 `axial=75-90`、少量 coronal/sagittal top/near-top；这些 non-axial top-weight 应优先出现在 mixed-view / true-margin 支持样本上，从而把 seed42 learned full-fusion `val_acc` 推过 matched equal-weight `0.9255319148936170`，而不是机械平均或只改善 AUC。

- [x] **DFR-88-RESNEXT-DECISION-256X8-DISAGREEMENT-NONAXIAL-GATE-AUX-SEED42-MAIN-STUDY**：commit `020239d`，fresh adaptive `main-study` 使用 [autoresearch_logs/generated_search_configs/optuna_main_search_iter_0015_20260512_083335.yaml](/dataset/HH/ankle-ct/autoresearch_logs/generated_search_configs/optuna_main_search_iter_0015_20260512_083335.yaml)，study root [runs/optuna_main_autoloop/iter_0015_20260512_083335](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0015_20260512_083335)，GPU policy `--gpu-ids 0,2,3 --max-workers 3`，只跑 seed42 三个保守候选 `ANKLE_DECISION_GATE_VIEW_CORRECTNESS_AUX_WEIGHT=0.005/0.01/0.02`，并固定 disagreement + non-axial-only aux。实验实际结果：trial0 `0.9148936170212766/0.9781818181818183/0.9090909090909091`，trial1 `0.9148936170212766/0.9813636363636364/0.9111111111111111`，trial2 `0.9255319148936170/0.9622727272727272/0.9195402298850575`；best by `val_acc` 为 trial2，`peak_vram≈2.19 GiB`, `total_seconds≈614.8` → **discard**（best seed42 只打平 matched equal-weight seed42，仍低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`；AUC/F1 也未超过 DFR-25。）
- **fusion-weight 结论**：best trial2 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0015_20260512_083335/trials/trial_0002/run/fusion_weight_analysis.json) 显示 DFR-88 确实移动了 routing，但没有形成 right-view-at-right-sample 的净收益。mean fusion weight `axial/coronal/sagittal = 0.6735/0.1404/0.1861`；top-weight count/rate `axial=81/0.8617`, `coronal=0/0.0000`, `sagittal=13/0.1383`；`top_true_margin` 分布为 `axial/coronal/sagittal = 84/4/6`，`top_weight_hit_rate.true_margin=0.7553`。也就是说，routing 从 DFR-87 的 hard axial lock-in 中释放出来，但它把 13 个样本迁到 sagittal，而 true-margin 只支持 6 个 sagittal 样本；同时 4 个 coronal true-margin 样本没有获得任何 top-weight。coronal/sagittal 单视角 accuracy 均只有 `0.5319`，迁移没有转化为正确补充证据。
- **当前判断**：DFR-88 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它把 DFR-86 的释放和 DFR-84 的 disagreement mask结合后仍只达到 equal-weight tie，说明现有 gate view-correctness aux family 无法区分“应释放的 oracle-correct non-axial error”与“会打坏 DFR-25-correct 样本的弱视角迁移”。下一轮不要继续扫 DFR-83/86/88 aux weight、base scale、disagreement 或 axial-protection阈值；若外层 loop 继续，应先做能在不改 `train.py` 的前提下表达 error-aware selector / pairwise target 的可行性设计，或继续 analysis-only 审计。
- **Agent 状态（2026-05-12 08:55 SGT）**：本 session 是唯一 coordinator，已完成 exactly one research iteration；last experiment=`DFR-88 disagreement-gated non-axial gate view-correctness aux`，lane=`main-study`，result=`discard`，last commit=`020239d`；下一步建议=`close simple gate-correctness aux combinations; if continuing, design an error-aware selector/ranking objective that can be expressed through the existing train.py aux hook or validate it offline first`，继续服务 DFR-25 decision-fusion routing 修复，不切 backbone / geometry / fusion family。

### DFR-89 pairwise selector auxiliary loss（2026-05-12）

> **实验说明**
> - 本轮继续从 DFR-25 anchor 出发；保持 ResNeXt 256x8、learned decision fusion、L3 no-mixer、dominant-gate dropout、winning scalar 与数据划分不变。
> - 唯一新增机制是在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 通过 `ANKLE_DECISION_ENABLE_PAIRWISE_SELECTOR_AUX_LOSS=1` 暴露一个 pairwise selector aux：分别构造 `axial+coronal` 与 `axial+sagittal` 两个候选融合 logits，候选只让 normalized pair fusion weights 接收梯度，per-view logits detach；同时固定 `ANKLE_DECISION_PAIRWISE_SELECTOR_AUX_REQUIRE_DISAGREEMENT=1`，只在 non-axial 与 axial 预测类别不同的 pair 上训练 selector。
> - 设计思路：DFR-88 的 non-axial aux 会把 sagittal 放出来，但不是 right-view-at-right-sample；DFR-89 改成二选一 pair selector，让 CE 直接惩罚错误 pair 融合，希望 gate 学会“当 axial 与某个非 axial 冲突时，这个 pair 应不应该相信非 axial”，而不是无条件放大全部 non-axial。
> - 预计改进效果：相对 DFR-88 best 的 `top_weight=81/0/13`，预期 DFR-89 应减少错误 sagittal 迁移，同时不回到 DFR-84/87 的 `94/0/0` hard axial；理想权重是 axial-majority、少量 coronal/sagittal top/near-top，并且 top-weight 更接近 true-margin 分布，从而把 seed42 learned full-fusion `val_acc` 推过 matched equal-weight `0.9255319148936170`。

- [x] **DFR-89-RESNEXT-DECISION-256X8-PAIRWISE-SELECTOR-AUX-SEED42-MAIN-STUDY**：commit `3778d94`，fresh adaptive `main-study` 使用 [configs/optuna_main_search_resnext_decision_256x8_dfr89_pairwise_selector_aux.yaml](/dataset/HH/ankle-ct/configs/optuna_main_search_resnext_decision_256x8_dfr89_pairwise_selector_aux.yaml)，study root [runs/optuna_main_resnext_decision_256x8_dfr89_pairwise_selector_aux](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr89_pairwise_selector_aux)，Slurm job `483653` 在 `PA100q/node03` 完成，GPU policy `--gpu-ids auto --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_PAIRWISE_SELECTOR_AUX_WEIGHT=0.0025/0.005/0.01`。实验实际结果：trial0 `0.9255319148936170/0.9686363636363636/0.9195402298850575`，trial1 `0.9255319148936170/0.9731818181818181/0.9230769230769231`，trial2 `0.9255319148936170/0.9804545454545455/0.9230769230769231`；best by `val_acc` then `val_auc` 为 trial2，`peak_vram≈2.19 GiB`, `total_seconds≈607.4` → **discard**（best seed42 只打平 matched equal-weight seed42，仍低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`；AUC 较高但主指标未达标。）
- **fusion-weight 结论**：best trial2 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr89_pairwise_selector_aux/trials/trial_0002/run/fusion_weight_analysis.json) 显示 DFR-89 没有推开 axial lock-in。mean fusion weight `axial/coronal/sagittal = 0.9112/0.0316/0.0572`；top-weight count/rate `axial=94/1.0000`, `coronal=0/0.0000`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 84/2/8`，`top_weight_hit_rate.true_margin=0.8936`。coronal/sagittal 单视角 accuracy 均为 `0.5319` 且 F1 为 `0.0`，说明 pairwise selector aux 主要保留了 axial 分类器，而没有学到可用的非 axial 正类证据。
- **当前判断**：DFR-89 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它比 DFR-88 更保守，避免了错误 sagittal over-routing，但代价是完全回到 `94/0/0` axial top-weight；所以问题不是简单 pairwise 监督强度不够，而是 aux 目标没有让 coronal/sagittal 的正类证据变可用。下一轮应先尝试更直接的 non-axial 正类监督 / class-bias 修正，目标是让 coronal/sagittal 至少能在少量阳性样本上形成非零 F1，再谈 selector routing。
- **Agent 状态（2026-05-12 13:49 SGT）**：本 session 是唯一 coordinator，已完成用户要求的 `dfr89`；last experiment=`DFR-89 pairwise selector auxiliary loss`，lane=`main-study`，result=`discard`，last commit=`3778d94`；下一步建议=`DFR-90 non-axial positive-sensitive auxiliary / class-bias repair before another selector threshold sweep`，继续服务 DFR-25 decision-fusion routing 修复，不切 backbone / geometry / fusion family。

### DFR-90 non-axial abnormal auxiliary loss（2026-05-12）

> **实验说明**
> - 本轮继续从 DFR-25 anchor 出发；保持 ResNeXt 256x8、learned decision fusion、L3 no-mixer、dominant-gate dropout、winning scalar 与数据划分不变。
> - 唯一新增机制是在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 通过 `ANKLE_DECISION_ENABLE_NONAXIAL_ABNORMAL_AUX_LOSS=1` 暴露一个 train-time auxiliary：只取 coronal/sagittal 两个非 axial 视角，把 normal logit detach，只让 abnormal logit 接收 CE 梯度。
> - 设计思路：DFR-89 已证明 pairwise selector aux 没有让 coronal/sagittal 的正类证据变可用；DFR-90 因此先不继续调 selector，而是直接修非 axial 的 abnormal class evidence，让弱视角至少能在少量阳性样本上形成可分类信号，再评估 gate 是否能利用它。
> - 预计改进效果：相对 DFR-89 best 的 coronal/sagittal F1=`0.0/0.0` 与 top-weight `94/0/0`，预期 coronal/sagittal 至少出现非零 F1，并在 true-margin 支持样本上获得少量 top/near-top routing；如果这些非 axial 正类证据覆盖 DFR-25/DFR-89 的 oracle-fixable 错样本，则 seed42 learned full-fusion 有机会超过 matched equal-weight `0.9255319148936170`。

- [x] **DFR-90-RESNEXT-DECISION-256X8-NONAXIAL-ABNORMAL-AUX-SEED42-MAIN-STUDY**：commit `fef9069`，fresh adaptive `main-study` 使用 [configs/optuna_main_search_resnext_decision_256x8_dfr90_nonaxial_abnormal_aux.yaml](/dataset/HH/ankle-ct/configs/optuna_main_search_resnext_decision_256x8_dfr90_nonaxial_abnormal_aux.yaml)，study root [runs/optuna_main_resnext_decision_256x8_dfr90_nonaxial_abnormal_aux](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr90_nonaxial_abnormal_aux)，Slurm job `483684` 在 `PA100q/node03` 完成，GPU policy `--gpu-ids auto --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_NONAXIAL_ABNORMAL_AUX_WEIGHT=0.01/0.025/0.05`。实验实际结果：trial0 `0.9148936170212766/0.9768181818181818/0.9111111111111111`，trial1 `0.9148936170212766/0.9822727272727273/0.9047619047619048`，trial2 `0.9255319148936170/0.9677272727272728/0.9176470588235294`；best by `val_acc` 为 trial2，`peak_vram≈2.19 GiB`, `total_seconds≈607.0` → **discard**（best seed42 只打平 matched equal-weight seed42，仍低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`；AUC/F1 也未超过 DFR-25。）
- **fusion-weight 结论**：best trial2 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr90_nonaxial_abnormal_aux/trials/trial_0002/run/fusion_weight_analysis.json) 显示 DFR-90 只修到一半。mean fusion weight `axial/coronal/sagittal = 0.8661/0.0894/0.0445`；top-weight count/rate `axial=94/1.0000`, `coronal=0/0.0000`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 84/8/2`，`top_weight_hit_rate.true_margin=0.8936`。好消息是 coronal 单视角从 DFR-89 的 F1 `0.0` 提升到 `0.6847`，说明 positive evidence repair 有信号；坏消息是 gate 仍然一次也没有把 top weight 给 coronal/sagittal，sagittal 仍是 F1 `0.0`，所以没有转化成 full-fusion 收益。
- **当前判断**：DFR-90 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它说明“同时推 coronal+sagittal abnormal logit”太粗：coronal 确实获得可用正类信号，但 sagittal 仍弱且 gate 仍 fully axial。下一轮应把变量再收窄为 **coronal-only abnormal auxiliary / class-bias repair**，先验证能否把 DFR-90 的 coronal F1 信号转成少量 evidence-aligned routing，而不是继续联合拉动 sagittal。
- **Agent 状态（2026-05-12 14:09 SGT）**：本 session 是唯一 coordinator，已完成用户要求的 `dfr89` 后续第 `1/6` 轮；last experiment=`DFR-90 non-axial abnormal auxiliary loss`，lane=`main-study`，result=`discard`，last commit=`fef9069`；下一步建议=`DFR-91 coronal-only abnormal auxiliary / class-bias repair, keep sagittal untouched`，继续服务 DFR-25 decision-fusion routing 修复，不切 backbone / geometry / fusion family。

### DFR-91 coronal-only abnormal auxiliary loss（2026-05-12）

> **实验说明**
> - 本轮继续从 DFR-25 anchor 出发；保持 ResNeXt 256x8、learned decision fusion、L3 no-mixer、dominant-gate dropout、winning scalar 与数据划分不变。
> - 唯一新增变量是在 DFR-90 的 non-axial abnormal aux 上加 `ANKLE_DECISION_NONAXIAL_ABNORMAL_AUX_CORONAL_ONLY=1`，只训练 coronal abnormal logit，sagittal 不进入该 auxiliary。
> - 设计思路：DFR-90 显示 coronal 获得了非零 F1，但 sagittal 仍是 F1 `0.0`，且 gate 继续 fully axial。DFR-91 因此不再联合推动 sagittal，先验证“只修 coronal 正类证据”能否减少噪声并把少量 coronal true-margin 样本释放到 routing。
> - 预计改进效果：相对 DFR-90 的 `top_true_margin coronal=8` 但 `top_weight coronal=0`，预期 coronal 单视角 F1 保持非零，并在少数 coronal true-margin 样本上获得 top/near-top weight；理想形态是 axial 仍为主导但不再 `94/0/0`，从而让 seed42 learned full-fusion 超过 matched equal-weight `0.9255319148936170`。

- [x] **DFR-91-RESNEXT-DECISION-256X8-CORONAL-ABNORMAL-AUX-SEED42-MAIN-STUDY**：commit `1ec2a2d`，fresh adaptive `main-study` 使用 [configs/optuna_main_search_resnext_decision_256x8_dfr91_coronal_abnormal_aux.yaml](/dataset/HH/ankle-ct/configs/optuna_main_search_resnext_decision_256x8_dfr91_coronal_abnormal_aux.yaml)，study root [runs/optuna_main_resnext_decision_256x8_dfr91_coronal_abnormal_aux](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr91_coronal_abnormal_aux)，Slurm job `483715` 在 `PA100q/node03` 完成，GPU policy `--gpu-ids auto --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_NONAXIAL_ABNORMAL_AUX_WEIGHT=0.01/0.025/0.05` 且固定 coronal-only。实验实际结果：trial0 `0.9255319148936170/0.9800000000000000/0.9195402298850575`，trial1 `0.9148936170212766/0.9822727272727272/0.9000000000000000`，trial2 `0.9255319148936170/0.9736363636363636/0.9213483146067416`；best by `val_acc` then `val_auc` 为 trial0，`peak_vram≈2.19 GiB`, `total_seconds≈609.4` → **discard**（best seed42 仍只打平 matched equal-weight seed42，低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`；AUC 虽高但主指标未达标。）
- **fusion-weight 结论**：best trial0 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr91_coronal_abnormal_aux/trials/trial_0000/run/fusion_weight_analysis.json) 显示 coronal-only aux 确实强化了 coronal classifier，但没有改变 gate top-rank。mean fusion weight `axial/coronal/sagittal = 0.8455/0.0646/0.0899`；top-weight count/rate `axial=94/1.0000`, `coronal=0/0.0000`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 85/4/5`，`top_weight_hit_rate.true_margin=0.9043`。coronal 单视角达到 `accuracy=0.7766`, `F1=0.6957`，但 gate 仍没有一次选 coronal，说明当前 aux 主要改好了独立分类器，没有改好 reliability logits 的排序。
- **当前判断**：DFR-91 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它确认 coronal positive evidence 可以被训练出来，但 full-fusion 仍卡在 axial lock-in；下一轮不应再加大 coronal aux，也不应回到 sagittal 联合训练。更合理的是测试 **更低权重的 coronal-only aux**，看能否保住 DFR-25 主干 accuracy 与 coronal signal，同时避免 DFR-91 中高权重对主融合的轻微扰动。
- **Agent 状态（2026-05-12 14:27 SGT）**：本 session 是唯一 coordinator，已完成用户要求的 `dfr89` 后续第 `2/6` 轮；last experiment=`DFR-91 coronal-only abnormal auxiliary loss`，lane=`main-study`，result=`discard`，last commit=`1ec2a2d`；下一步建议=`DFR-92 low-weight coronal-only abnormal auxiliary, test 0.001/0.0025/0.005`，继续服务 DFR-25 decision-fusion routing 修复，不切 backbone / geometry / fusion family。

### DFR-92 low-weight coronal-only abnormal auxiliary loss（2026-05-12）

> **实验说明**
> - 本轮继续从 DFR-91 出发；保持 ResNeXt 256x8、learned decision fusion、L3 no-mixer、dominant-gate dropout、winning scalar 与数据划分不变。
> - 不新增模型代码，只把 coronal-only abnormal aux weight 从 DFR-91 的 `0.01/0.025/0.05` 降到 `0.001/0.0025/0.005`。
> - 设计思路：DFR-91 把 coronal 单视角训练到 `accuracy=0.7766/F1=0.6957`，但 full-fusion 仍没有超过 equal-weight 且 gate 仍 `94/0/0`。DFR-92 检查一个更简单的可能性：是不是 aux 权重仍太重，扰动了 DFR-25 主干；如果是，低权重应保住主 accuracy，同时留下足够 coronal 正类信号。
> - 预计改进效果：相对 DFR-91 的 `val_acc=0.9255` 与 coronal F1 `0.6957`，预期低权重至少不低于 `0.9255`，理想情况下回到 DFR-25 seed42 `0.9362` 档，同时 top/near-top routing 对 coronal true-margin 样本略有释放。

- [x] **DFR-92-RESNEXT-DECISION-256X8-LOW-CORONAL-ABNORMAL-AUX-SEED42-MAIN-STUDY**：commit `ae95cd3`，fresh adaptive `main-study` 使用 [configs/optuna_main_search_resnext_decision_256x8_dfr92_coronal_abnormal_aux_low.yaml](/dataset/HH/ankle-ct/configs/optuna_main_search_resnext_decision_256x8_dfr92_coronal_abnormal_aux_low.yaml)，study root [runs/optuna_main_resnext_decision_256x8_dfr92_coronal_abnormal_aux_low](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr92_coronal_abnormal_aux_low)，Slurm job `483730` 在 `PA100q/node03` 完成，GPU policy `--gpu-ids auto --max-workers 3`，只跑 seed42 三个低权重候选 `ANKLE_DECISION_NONAXIAL_ABNORMAL_AUX_WEIGHT=0.001/0.0025/0.005` 且固定 coronal-only。实验实际结果：trial0 `0.9148936170212766/0.9768181818181818/0.9130434782608695`，trial1 `0.9148936170212766/0.9781818181818182/0.9090909090909091`，trial2 `0.9148936170212766/0.9777272727272728/0.9111111111111111`；best by `val_acc` then `val_auc` 为 trial1，`peak_vram≈2.19 GiB`, `total_seconds≈602.7` → **discard**（所有低权重候选均低于 matched equal-weight seed42 `0.9255319148936170` 与 DFR-25 seed42 `0.9361702127659575`。）
- **fusion-weight 结论**：best trial1 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr92_coronal_abnormal_aux_low/trials/trial_0001/run/fusion_weight_analysis.json) 显示低权重也没有打开 gate。mean fusion weight `axial/coronal/sagittal = 0.8701/0.0743/0.0556`；top-weight count/rate `axial=94/1.0000`, `coronal=0/0.0000`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 84/4/6`，`top_weight_hit_rate.true_margin=0.8936`。coronal 单视角仍有 `accuracy=0.7447`, `F1=0.7000`，但 full-fusion 主指标反而降到 `0.9149`。
- **当前判断**：DFR-92 是 negative seed42 candidate study，基本关闭“simple coronal aux weight sweep”这条支路。最近三轮的共同结论是：coronal positive evidence 可以被训练出来，但 reliability gate 的 top-rank 仍完全 axial；下一轮不应继续扫 abnormal aux 权重，而应把 coronal evidence 与 gate 排序显式连接，例如 **coronal abnormal aux + axial/coronal pair selector aux**，让 coronal 分类器与 gate 权重同时得到一致监督。
- **Agent 状态（2026-05-12 14:43 SGT）**：本 session 是唯一 coordinator，已完成用户要求的 `dfr89` 后续第 `3/6` 轮；last experiment=`DFR-92 low-weight coronal-only abnormal auxiliary loss`，lane=`main-study`，result=`discard`，last commit=`ae95cd3`；下一步建议=`DFR-93 coronal abnormal aux plus axial-coronal pair selector aux, explicitly connect coronal evidence to gate ranking`，继续服务 DFR-25 decision-fusion routing 修复，不切 backbone / geometry / fusion family。

### DFR-93 coronal pair abnormal auxiliary loss（2026-05-12）

> **实验说明**
> - 本轮继续从 DFR-25 anchor 出发；保持 ResNeXt 256x8、learned decision fusion、L3 no-mixer、dominant-gate dropout、winning scalar 与数据划分不变。
> - 唯一新增组合机制是在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 通过 `ANKLE_DECISION_ENABLE_CORONAL_PAIR_ABNORMAL_AUX_LOSS=1` 同时暴露两个 train-time auxiliary：一个训练 coronal abnormal logit，另一个只在 axial-coronal pair 内训练 pair-normalized fusion weight；per-view logits detach，让梯度主要作用到 gate/pair weight。
> - 设计思路：DFR-90/91/92 已说明 coronal 分类器可以被训练出正类信号，但 gate 不会自动选 coronal；DFR-93 把 coronal evidence repair 和 axial-coronal pair selector 合在一起，直接连接“coronal 有证据”和“gate 应该在 axial/coronal pair 里给它权重”。
> - 预计改进效果：相对 DFR-92 的 `top_weight=94/0/0`，预期 axial 仍占多数，但 coronal 在少数 true-margin 支持样本上获得 top/near-top routing；如果这些样本覆盖 DFR-25 的可修复错例，则 seed42 learned full-fusion 有机会超过 matched equal-weight `0.9255319148936170`。

- [x] **DFR-93-RESNEXT-DECISION-256X8-CORONAL-PAIR-ABNORMAL-AUX-SEED42-MAIN-STUDY**：commit `c16a69e`，fresh adaptive `main-study` 使用 [configs/optuna_main_search_resnext_decision_256x8_dfr93_coronal_pair_abnormal_aux.yaml](/dataset/HH/ankle-ct/configs/optuna_main_search_resnext_decision_256x8_dfr93_coronal_pair_abnormal_aux.yaml)，study root [runs/optuna_main_resnext_decision_256x8_dfr93_coronal_pair_abnormal_aux](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr93_coronal_pair_abnormal_aux)，Slurm job `483742` 在 `PA100q/node03` 完成，GPU policy `--gpu-ids auto --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_CORONAL_PAIR_ABNORMAL_AUX_WEIGHT=0.0025/0.005/0.01`。实验实际结果：trial0 `0.9255319148936170/0.9668181818181818/0.9195402298850575`，trial1 `0.9148936170212766/0.9850000000000000/0.9090909090909091`，trial2 `0.9148936170212766/0.9777272727272728/0.9111111111111111`；best by `val_acc` 为 trial0，`peak_vram≈2.19 GiB`, `total_seconds≈607.2` → **discard**（best seed42 只打平 matched equal-weight seed42，仍低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`；trial1 AUC 高但主指标更低，不能用于选择。）
- **fusion-weight 结论**：best trial0 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr93_coronal_pair_abnormal_aux/trials/trial_0000/run/fusion_weight_analysis.json) 显示 DFR-93 第一次明显推开了 hard axial gate，但方向不对。mean fusion weight `axial/coronal/sagittal = 0.6414/0.1179/0.2407`；top-weight count/rate `axial=72/0.7660`, `coronal=0/0.0000`, `sagittal=22/0.2340`；`top_true_margin` 分布为 `axial/coronal/sagittal = 84/4/6`，`top_weight_hit_rate.true_margin=0.6809`。coronal 单视角退回 `accuracy=0.5319`, `F1=0.0`，sagittal 也为 `F1=0.0`，说明 gate 被释放了，但释放到了错误视角。
- **当前判断**：DFR-93 是 negative seed42 candidate study，不应 promotion 到 3-seed formal；但它给出一个有用信号：pair-style auxiliary 能移动 gate，只是当前三视角竞争下 sagittal 会抢走权重。下一轮应直接测试 **forced axial+coronal active mask / no sagittal competition**，保留 DFR-93 的 axial-coronal pair 监督，但在最终融合中临时排除 sagittal，验证 coronal 是否能在没有 sagittal 干扰时转化为主指标收益。
- **Agent 状态（2026-05-12 18:00 SGT）**：本 session 是唯一 coordinator，已完成用户要求的 `dfr89` 后续第 `4/6` 轮；last experiment=`DFR-93 coronal pair abnormal auxiliary loss`，lane=`main-study`，result=`discard with routing signal`，last commit=`c16a69e`；下一步建议=`DFR-94 forced axial-coronal active mask with DFR-93 aux, remove sagittal competition before changing objective again`，继续服务 DFR-25 decision-fusion routing 修复，不切 backbone / geometry / fusion family。

### DFR-94 forced axial-coronal active mask with pair aux（2026-05-12）

> **实验说明**
> - 本轮继续从 DFR-93 出发；保持 ResNeXt 256x8、learned decision fusion、L3 no-mixer、dominant-gate dropout、winning scalar 与数据划分不变。
> - 唯一行为变量是在 DFR-93 coronal-pair aux 之上加 `ANKLE_DECISION_FORCE_ACTIVE_VIEW_MASK=1,1,0`，让最终融合只使用 axial+coronal，排除 DFR-93 中错误抢权重的 sagittal；同时 [scripts/analyze_fusion_weights.py](/dataset/HH/ankle-ct/scripts/analyze_fusion_weights.py) 记录 active-mask 重新归一化后的有效 fusion weight，并保留 raw weight。
> - 设计思路：DFR-93 证明 pair aux 能移动 gate，但权重跑向 sagittal。DFR-94 先不换主干、不换融合家族，只拿掉 sagittal 竞争，检查 coronal 在 axial-coronal 二视角竞争里是否能变成实际收益。
> - 预计改进效果：相对 DFR-93 的 `top_weight=72/0/22`，预期有效 top-weight 应只在 axial/coronal 之间分配，并在 coronal true-margin 样本上给 coronal 少量 top/near-top 权重；如果 DFR-93 的主要问题只是 sagittal 干扰，则 seed42 learned full-fusion 应超过 matched equal-weight `0.9255319148936170`。

- [x] **DFR-94-RESNEXT-DECISION-256X8-FORCED-AXCOR-PAIR-AUX-SEED42-MAIN-STUDY**：commit `11e76ff`，fresh adaptive `main-study` 使用 [configs/optuna_main_search_resnext_decision_256x8_dfr94_forced_axcor_pair_aux.yaml](/dataset/HH/ankle-ct/configs/optuna_main_search_resnext_decision_256x8_dfr94_forced_axcor_pair_aux.yaml)，study root [runs/optuna_main_resnext_decision_256x8_dfr94_forced_axcor_pair_aux](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr94_forced_axcor_pair_aux)，Slurm job `484388` 在 `PA100q/node03` 完成，GPU policy `--gpu-ids auto --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_CORONAL_PAIR_ABNORMAL_AUX_WEIGHT=0.001/0.0025/0.005` 且固定 active mask `1,1,0`。实验实际结果：trial0 `0.9148936170212766/0.9722727272727272/0.9130434782608695`，trial1 `0.9255319148936170/0.9759090909090910/0.9230769230769231`，trial2 `0.9148936170212766/0.9777272727272728/0.9111111111111111`；best by `val_acc` 为 trial1，`peak_vram≈2.19 GiB`, `total_seconds≈612.7` → **discard**（best seed42 只打平 matched equal-weight seed42，仍低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`。）
- **fusion-weight 结论**：best trial1 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_resnext_decision_256x8_dfr94_forced_axcor_pair_aux/trials/trial_0001/run/fusion_weight_analysis.json) 显示 DFR-94 没有释放 coronal。effective mean fusion weight（active mask 后）`axial/coronal/sagittal = 0.9993/0.0007/0.0000`，raw mean weight `0.7207/0.0004/0.2789`；effective top-weight `94/0/0`。`top_true_margin` 分布为 `65/27/2`，说明有不少样本 coronal true margin 更强，但 gate 仍几乎完全给 axial。coronal 单视角退化为 `accuracy=0.5319`, `F1=0.0`, `AUC=0.6355`。
...
## 2026-05-12：Decision-Fusion Repair Follow-up（DFR-97 sagittal-normal rescue aux，main-study）

> **DFR-97 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-96 把可救目标收缩到极少数 sagittal-normal FP rescue 样本；DFR-97 不再释放 coronal、不做 active mask，也不让 non-axial 全局接管，只在训练态通过 axial+sagittal pair CE 给 sagittal normal evidence 一个很小的 gate reliability credit。
> - 如果成功，为什么有机会把 learned full-fusion `val_acc` 推到 matched `equal-weight` 之上？因为推理期仍是三视角 learned decision fusion，且预期 top-weight 分布不是平均化，而是从 DFR-25 seed42 的 `94/0/0` 只迁出约 `3-4` 个 validated sagittal-normal rescue 样本；若这些迁移覆盖 DFR-25 的 FP 错误，同时保护 axial-positive / high-axial-margin 样本，就有机会在保留 DFR-25 主体收益的基础上超过 matched equal-weight。

- [x] **DFR-97-RESNEXT-DECISION-256X8-SAGITTAL-NORMAL-RESCUE-AUX-SEED42-MAIN-STUDY**：commit `2ac9151`，在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增默认关闭的 `ANKLE_DECISION_ENABLE_SAGITTAL_NORMAL_RESCUE_AUX_LOSS=1`，只在 detached axial 预测 abnormal 且 sagittal 预测 normal 的样本上，用 axial+sagittal pair logits 和 learned pair weights 构造训练期 auxiliary CE；推理期 learned decision fusion 不变。fresh adaptive `main-study` 使用 [autoresearch_logs/generated_search_configs/optuna_main_search_iter_0002_20260512_203844.yaml](/dataset/HH/ankle-ct/autoresearch_logs/generated_search_configs/optuna_main_search_iter_0002_20260512_203844.yaml)，study root [runs/optuna_main_autoloop/iter_0002_20260512_203844](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0002_20260512_203844)，GPU policy `--gpu-ids 0,1,2 --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_SAGITTAL_NORMAL_RESCUE_AUX_WEIGHT=0.001/0.0025/0.005`。设计思路：把 DFR-96 的 tiny sagittal-normal target 转成最小训练期 gate 校准，不再 broad release non-axial，也不触碰 backbone / geometry / fusion family。预计改进效果：全局 top-weight 应保持 axial 绝对多数，理想形态约 `axial=90/94, coronal=0/94, sagittal=4/94`，只让 validated sagittal-normal FP rescue 样本获得 top/near-top routing，从而把 learned full-fusion seed42 `val_acc` 推过 matched equal-weight `0.9255319148936170`。实验实际结果：trial0 `0.9255319148936170/0.9704545454545455/0.9230769230769231`，trial1 `0.9255319148936170/0.9740909090909091/0.9213483146067416`，trial2 `0.9148936170212766/0.9709090909090909/0.9130434782608695`；best by `val_acc` then `val_auc` 为 trial1，`peak_vram≈2.15 GiB`, `total_seconds≈792.9` → **discard**（best seed42 只打平 matched equal-weight seed42，低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`。）
- **fusion-weight 结论**：best trial1 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0002_20260512_203844/trials/trial_0001/run/fusion_weight_analysis.json) 显示 DFR-97 没有按 DFR-96 target 释放 sagittal。mean fusion weight `axial/coronal/sagittal = 0.7116/0.1860/0.1024`；top-weight count/rate `axial=81/0.8617`, `coronal=13/0.1383`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 84/3/7`，`top_weight_hit_rate.true_margin=0.7553`。也就是说，aux 把 gate 从 hard axial 推开了，但迁移到了 coronal 而不是 DFR-96 推荐的 sagittal-normal 小集合，且 sagittal top-weight 仍为 `0/94`。
- **当前判断**：DFR-97 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它否定了“只用 sagittal-normal pair CE 就能把 DFR-96 tiny target 写进 gate”的假设；训练梯度仍会通过共享 reliability scorer 引起 coronal side-effect，不能精准释放 sagittal。下一轮不应继续简单调 `sagittal_normal_rescue_aux_weight`，也不应回到 coronal/pair/active-mask sweep；若继续主线，应先做更直接的 calibration-only / post-training gate-bias audit，或设计显式 view-index constrained 的 sagittal logit correction，目标必须避免 coronal 被动释放。

## 2026-05-12：Decision-Fusion Repair Follow-up（DFR-98 sagittal-normal rescue gate，main-study）

> **DFR-98 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-96 把目标收缩到 tiny sagittal-normal FP rescue，DFR-97 证明训练期 pair CE 会产生 coronal side-effect；DFR-98 因此改成默认关闭、eval-only、显式 view-index constrained 的 sagittal-only logit residual gate，只在 detached axial abnormal / sagittal normal / fused abnormal / axial top-confidence 同时成立时触发。
> - 如果成功，为什么有机会把 learned full-fusion `val_acc` 推到 matched `equal-weight` 之上？因为它不平均三视角、不释放 coronal，也不改变 view classifier；预期 top-weight 仍保持 axial 绝对多数，只把极少数 axial-FP / sagittal-normal rescue 样本迁向 sagittal near-top/top，从而修复 DFR-25 seed42 的边界假阳性，同时保留 DFR-25 已超过 equal-weight 的主体 routing。

- [x] **DFR-98-RESNEXT-DECISION-256X8-SAGITTAL-NORMAL-RESCUE-GATE-SEED42-MAIN-STUDY**：commit `ed507f4`，在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增默认关闭的 `SagittalNormalRescueGate`，通过 `ANKLE_DECISION_ENABLE_SAGITTAL_NORMAL_RESCUE_GATE=1` 启用；fresh adaptive `main-study` 使用 [autoresearch_logs/generated_search_configs/optuna_main_search_iter_0003_20260512_210836.yaml](/dataset/HH/ankle-ct/autoresearch_logs/generated_search_configs/optuna_main_search_iter_0003_20260512_210836.yaml)，study root [runs/optuna_main_autoloop/iter_0003_20260512_210836](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0003_20260512_210836)，GPU policy `--gpu-ids 0,1,2 --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_SAGITTAL_NORMAL_RESCUE_GATE_RESIDUAL_LIMIT=0.25/0.50/0.75`。设计思路：把 DFR-96 tiny target 做成 inference-time sagittal-only gate calibration，避免 DFR-97 的训练期共享 scorer side-effect。预计改进效果：top-weight 分布应接近 axial 仍 `90+/94`、coronal `0/94`、sagittal 约 `3-4/94`，也就是只让 validated sagittal-normal FP rescue 样本获得 top/near-top routing，从而把 learned full-fusion seed42 `val_acc` 推过 matched equal-weight `0.9255319148936170` 并尝试超过 DFR-25 seed42。实验实际结果：trial0 `0.9148936170212766/0.9695454545454546/0.9111111111111111`，trial1 `0.9361702127659575/0.9740909090909091/0.9318181818181818`，trial2 `0.9255319148936170/0.9777272727272728/0.9213483146067416`；best by `val_acc` then `val_auc` 为 trial1，`peak_vram≈2.20 GiB`, `total_seconds≈759.7` → **discard**（best seed42 主指标只打平 DFR-25 seed42 `0.9361702127659575`，但 `val_auc` 低于 DFR-25 seed42 `0.9786363636363636`，不能按 tie-break 保留。）
...
## 2026-05-12：Decision-Fusion Repair Follow-up（DFR-100 gate pairwise contrast aux，main-study）

> **DFR-100 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-99 已经否定手写 sagittal trigger，本轮改成训练期 learned routing 约束：在 axial/coronal、axial/sagittal、coronal/sagittal 三个 pair 内，用 detached per-view logits 与当前 learned pair-normalized fusion weights 构造 pair candidate logits，只在两视角预测不同的样本上给 gate 反向信号。
> - 如果成功，为什么有机会把 learned full-fusion `val_acc` 推到 matched `equal-weight` 之上？因为推理期仍是普通 learned full-fusion，不固定平均权重，也不让 aux 改 view classifier；强 axial all-agree 样本不受 disagreement-gated pair aux 直接驱动，而 disagreement 样本中真正更可靠的 non-axial view 应获得 top/near-top routing。预期 routing 是从 DFR-25 seed42 `94/0/0` 迁到 axial 仍主导、coronal/sagittal 只在少量有证据样本接管，而不是机械均权。

- [x] **DFR-100-RESNEXT-DECISION-256X8-GATE-PAIRWISE-CONTRAST-AUX-SEED42-MAIN-STUDY**：commit `2384467`，在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增默认关闭的 `ANKLE_DECISION_ENABLE_GATE_PAIRWISE_CONTRAST_AUX_LOSS=1`，以 `ANKLE_DECISION_GATE_PAIRWISE_CONTRAST_AUX_REQUIRE_DISAGREEMENT=1` 限制 aux 只作用于 pair prediction disagreement；fresh adaptive `main-study` 使用 [autoresearch_logs/generated_search_configs/optuna_main_search_iter_0005_20260512_220517.yaml](/dataset/HH/ankle-ct/autoresearch_logs/generated_search_configs/optuna_main_search_iter_0005_20260512_220517.yaml)，study root [runs/optuna_main_autoloop/iter_0005_20260512_220517](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0005_20260512_220517)，GPU policy `--gpu-ids 0,1,2 --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_GATE_PAIRWISE_CONTRAST_AUX_WEIGHT=0.001/0.0025/0.005`。设计思路：把 DFR-99 之后的方向从失败的可观测 trigger 改为 learned pairwise routing repair，aux 的 candidate pair logits 使用 detached view logits 与 learned pair weights，因此主要训练 gate 的相对排序，不直接塑形 per-view classifier。预计改进效果：axial 仍应是多数 top-weight，但 disagreement 样本中 coronal/sagittal 若提供正确证据，应从 DFR-25 的 `94/0/0` 迁出少量 top/near-top；理想形态是 `axial≈85-90/94` 且 non-axial top-weight 覆盖 top-true-margin 支持样本，从而让 learned full-fusion 超过 matched equal-weight seed42 `0.9255319148936170` 并尝试超过 DFR-25 seed42。实验实际结果：trial0 `0.9255319148936170/0.9750000000000000/0.9230769230769231`，trial1 `0.9148936170212766/0.9827272727272727/0.9090909090909091`，trial2 `0.9255319148936170/0.9809090909090910/0.9230769230769231`；best by `val_acc` then `val_auc` 为 trial2，`peak_vram≈2.15 GiB`, `total_seconds≈764.9` → **discard**（best seed42 主指标只打平 matched equal-weight seed42，低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`；虽然 AUC 高于 DFR-25，但协议不能用 AUC 覆盖 val_acc 回落。）
- **fusion-weight 结论**：best trial2 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0005_20260512_220517/trials/trial_0002/run/fusion_weight_analysis.json) 显示 pairwise contrast aux 确实削弱 hard axial lock-in，但迁移方向仍不满足 DFR-96/99 目标。mean fusion weight `axial/coronal/sagittal = 0.7546/0.1687/0.0767`；top-weight count/rate `axial=86/0.9149`, `coronal=8/0.0851`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 84/3/7`，`top_weight_correct_rate=0.8404`。也就是说，本轮把部分 top-routing 从 axial 迁到 coronal，但仍没有释放 sagittal top-weight；routing 变得更分散，却没有形成比 matched equal-weight 更好的 full-fusion accuracy。
- **当前判断**：DFR-100 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它验证了 pairwise learned aux 能移动 gate，但移动到 coronal 的 8 个 top-weight 样本没有带来净 accuracy 增益，且 sagittal 仍为 `0/94`。下一轮若继续主线，不应继续只扫 pairwise contrast aux weight；更有信息量的方向是分析 DFR-100 coronal-top 样本与 DFR-96 sagittal target 的错位，或把 pairwise objective 改成带 validated target/protect 的 ranking 约束，避免“能迁移但迁错视角”。

---

## 2026-05-12：Decision-Fusion Repair Follow-up（DFR-101 axial-sagittal rank aux，main-study）

> **DFR-101 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-100 证明 pairwise learned aux 可以把 gate 从 axial 推出来，但迁移到了不增益的 coronal；DFR-101 因此只在 axial-sagittal pair 上加 rank auxiliary，并且只在 detached axial=abnormal、sagittal=normal 的样本上触发，显式排除 coronal side-effect。
> - 如果成功，为什么有机会把 learned full-fusion `val_acc` 推到 matched `equal-weight` 之上？因为推理期仍是普通 learned full-fusion，view logits 全部 detach 到 aux，且 expected routing 不是平均化：正常标签样本应给 sagittal-normal reliability 少量 top/near-top 机会，异常标签样本继续保护 axial-abnormal reliability。理想 top-weight 从 DFR-25 seed42 的 `94/0/0` 迁到约 `90/0/4`，覆盖 DFR-96 的 tiny sagittal-normal FP rescue 集合，同时保住 DFR-25 主体 axial-positive 通道。

- [x] **DFR-101-RESNEXT-DECISION-256X8-AXIAL-SAGITTAL-RANK-AUX-SEED42-MAIN-STUDY**：commit `f300ae3`，在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增默认关闭的 `ANKLE_DECISION_ENABLE_AXIAL_SAGITTAL_RANK_AUX_LOSS=1`；aux logits 只由 learned axial/sagittal pair weights 构成，class `0` 目标推高 sagittal reliability，class `1` 目标推高 axial reliability，且只在 detached axial 预测 abnormal、sagittal 预测 normal 时保留梯度。fresh adaptive `main-study` 使用 [autoresearch_logs/generated_search_configs/optuna_main_search_iter_0006_20260512_224440.yaml](/dataset/HH/ankle-ct/autoresearch_logs/generated_search_configs/optuna_main_search_iter_0006_20260512_224440.yaml)，study root [runs/optuna_main_autoloop/iter_0006_20260512_224440](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0006_20260512_224440)，GPU policy `--gpu-ids 0,1,2 --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_AXIAL_SAGITTAL_RANK_AUX_WEIGHT=0.0005/0.001/0.0025`。设计思路：把 DFR-100 的 broad pairwise contrast 改成 sagittal-target / axial-protect ranking constraint，避免继续“能移动但迁错视角”。预计改进效果：top-weight 应保持 axial 绝对多数，但不再是 `94/0/0`；预期少量 sagittal-normal FP rescue 样本获得 sagittal top/near-top，coronal top-weight 维持 `0/94`，从而给 learned full-fusion 超过 matched equal-weight seed42 留出样本级增益。实验实际结果：trial0 `0.9148936170212766/0.9786363636363637/0.9111111111111111`，trial1 `0.9148936170212766/0.9677272727272728/0.9130434782608695`，trial2 `0.9255319148936170/0.9759090909090910/0.9230769230769231`；best by `val_acc` then `val_auc` 为 trial2，`peak_vram≈2.15 GiB`, `total_seconds≈758.7` → **discard**（best seed42 只打平 matched equal-weight seed42 `0.9255319148936170`，低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`。）
- **fusion-weight 结论**：best trial2 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0006_20260512_224440/trials/trial_0002/run/fusion_weight_analysis.json) 显示 rank aux 没有按预期释放 sagittal，反而重新接近 axial collapse。mean fusion weight `axial/coronal/sagittal = 0.9096033097581661/0.07249687191970805/0.017899820105509556`；top-weight count/rate `axial=94/1.0000`, `coronal=0/0.0000`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 83/2/9`，`top_weight_hit_rate.true_margin=0.8829787234042553`。虽然 coronal side-effect 被抑制了，但 sagittal 仍没有获得任何 top routing，说明用 pair-weight CE 直接排序 axial/sagittal 仍不足以克服 base confidence gap。
- **当前判断**：DFR-101 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它说明“带 validated target/protect 语义的 axial-sagittal rank auxiliary”也无法把 DFR-96 的 tiny sagittal target 写进 gate；与 DFR-100 相比，它只是从 coronal over-migration 回到 axial re-collapse。下一轮不应继续扫 rank aux weight 或再做手写 sagittal trigger，应转向解释 base confidence gap 的来源：例如离线分析 DFR-25/100/101 中 sagittal-target 样本的 confidence logit 与 feature norm / view classifier margin，或设计不会只依赖当前 gate pair weights 的 learned sagittal reliability calibrator。

---

## 2026-05-12：Decision-Fusion Repair Follow-up（DFR-102 sagittal reliability calibrator，main-study）

> **DFR-102 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-101 说明 pair-weight CE 仍被 base gate confidence gap 压住；DFR-102 因此新增独立的 sagittal-only reliability residual calibrator，用 detached per-view logits、当前 fusion weights、margin / abnormal probability / fused context 学一个低容量 confidence residual，不改 view classifier，也不固定平均权重。
> - 如果成功，为什么有机会把 learned full-fusion `val_acc` 推到 matched `equal-weight` 之上？因为推理期仍是 learned full-fusion softmax，只给 sagittal confidence 一个 bounded residual。预期不是把三视角压平均，而是让 axial 保持多数，同时把 DFR-96/101 暴露的少量 sagittal evidence 样本推到 sagittal near-top/top；理想 top-weight 从 `94/0/0` 迁到约 `88-92/0-3/2-6`，用样本级 rescue 超过 matched equal-weight seed42。

- [x] **DFR-102-RESNEXT-DECISION-256X8-SAGITTAL-RELIABILITY-CALIBRATOR-SEED42-MAIN-STUDY**：commit `3d7519d`，在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增默认关闭的 `SagittalReliabilityCalibrator`，通过 `ANKLE_DECISION_ENABLE_SAGITTAL_RELIABILITY_CALIBRATOR=1` 启用；calibrator 的输入全部 detached，输出只加到 sagittal confidence logit，并由 `ANKLE_DECISION_SAGITTAL_RELIABILITY_CALIBRATOR_RESIDUAL_LIMIT` 限幅。fresh adaptive `main-study` 使用 [autoresearch_logs/generated_search_configs/optuna_main_search_iter_0007_20260512_232402.yaml](/dataset/HH/ankle-ct/autoresearch_logs/generated_search_configs/optuna_main_search_iter_0007_20260512_232402.yaml)，study root [runs/optuna_main_autoloop/iter_0007_20260512_232402](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0007_20260512_232402)，GPU policy `--gpu-ids 0,1,2 --max-workers 3`，只跑 seed42 三个候选 residual limit `0.25/0.5/1.0`。设计思路：把 DFR-101 的 sagittal target 从 pair-weight 排序改成独立 learned reliability 校准，让 sagittal 在有证据样本上能跨过 base confidence gap，同时避免手写 trigger 与 coronal side-effect。预计改进效果：axial 仍应保持 top-weight 主导，但不再是 `94/0/0`；预期少量 sagittal-normal / sagittal true-margin 样本进入 sagittal top/near-top，coronal 不应成为主要迁移方向，从而让 learned full-fusion seed42 `val_acc` 超过 matched equal-weight `0.9255319148936170` 并尝试超过 DFR-25 seed42。实验实际结果：trial0 `0.9255319148936170/0.9736363636363636/0.9213483146067416`，trial1 `0.9148936170212766/0.9690909090909092/0.9024390243902439`，trial2 `0.9042553191489362/0.9563636363636363/0.8860759493670886`；best by `val_acc` then `val_auc` 为 trial0，`peak_vram≈2.15 GiB`, `total_seconds≈771.2` → **discard**（best seed42 只打平 matched equal-weight seed42，低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333`。）
- **fusion-weight 结论**：best trial0 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0007_20260512_232402/trials/trial_0000/run/fusion_weight_analysis.json) 显示 calibrator 确实释放了 mean weight，但迁移方向仍不满足目标。mean fusion weight `axial/coronal/sagittal = 0.5577487625340198/0.2564358654967014/0.18581537117666386`；top-weight count/rate `axial=88/0.9362`, `coronal=6/0.0638`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 67/20/7`，`top_weight_hit_rate.true_margin=0.7553`。也就是说，本轮把 axial lock-in 松开了，但没有把任何样本交给 sagittal top routing，且 coronal 获得 6 个 top-weight；平均 sagittal mass 上升没有转化为目标 rescue 或 accuracy 增益。
- **当前判断**：DFR-102 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它比 DFR-101 更能改变权重分布，但仍没有解决 “sagittal 有 true-margin 支持却拿不到 top routing” 的核心问题；继续扫 residual limit 只会在 coronal leakage 与 accuracy 回落之间移动。下一轮若继续主线，应停止纯幅度版 sagittal calibrator sweep，转向更强的 evidence-consistency 约束或离线审计 calibrator 特征在 `top_true_margin=sagittal` 样本上的可分性，明确约束迁移必须命中 sagittal 而不是 coronal。
...
## 2026-05-13：Decision-Fusion Repair Follow-up（DFR-104 gate label-evidence rank auxiliary，main-study）

> **DFR-104 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-100 到 DFR-103 表明手写 trigger、pair rank、sagittal residual 与 coronal suppression 都不能稳定命中 non-axial true-margin 样本；本轮改用训练期 label-evidence rank auxiliary，把 detached per-view true-label evidence 与当前 raw learned fusion weights 做 log-mixture，只让梯度更新 gate 排序。
> - 如果成功，为什么有机会把 learned full-fusion `val_acc` 推到 matched `equal-weight` 之上？推理期仍是 learned full-fusion，不固定平均权重，也不直接改变 per-view classifier；预期是 axial 在自身证据最强时继续主导，但 coronal/sagittal 在 detached true-label evidence 更强的少量样本上获得 top/near-top，从样本级 right-view routing 超过 matched equal-weight seed42 `0.9255319148936170`。

- [x] **DFR-104-RESNEXT-DECISION-256X8-GATE-LABEL-EVIDENCE-RANK-AUX-SEED42-MAIN-STUDY**：commit `442e4a2`，在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增默认关闭的 `ANKLE_DECISION_ENABLE_GATE_LABEL_EVIDENCE_RANK_AUX_LOSS=1`；aux logits 使用 `logsumexp(log(raw_fusion_weight) + detached per-view log_prob)` 形成 label-evidence mixture，让现有训练 hook 的 CE 主要训练 gate 权重而不改变 per-view logits。fresh adaptive `main-study` 使用 [autoresearch_logs/generated_search_configs/optuna_main_search_iter_0009_20260513_002819.yaml](/dataset/HH/ankle-ct/autoresearch_logs/generated_search_configs/optuna_main_search_iter_0009_20260513_002819.yaml)，study root [runs/optuna_main_autoloop/iter_0009_20260513_002819](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0009_20260513_002819)，GPU policy `--gpu-ids 0,1,2 --max-workers 3`，只跑 seed42 三个候选 `ANKLE_DECISION_GATE_LABEL_EVIDENCE_RANK_AUX_WEIGHT=0.0005/0.001/0.0025`。设计思路：从 DFR-25 出发加一个 train-time gate label-evidence rank auxiliary，用 labels 只约束“哪个 view 的 detached evidence 应被 gate 采信”，避免再用手写 sagittal trigger 或单独 residual calibrator。预计改进效果：相对 DFR-103/DFR-25 的 axial-heavy routing，top-weight 应仍以 axial 为多数，但从 `94/0/0` 迁出少量证据支持样本到 coronal/sagittal；mean axial 应保持高位而不是均权化，若 true-label evidence 排序能命中 non-axial rescue 样本，就有机会把 learned full-fusion seed42 `val_acc` 推过 matched equal-weight `0.9255319148936170`。实验实际结果：trial0 `0.9148936170212766/0.9872727272727273/0.9069767441860465`，trial1 `0.9148936170212766/0.9759090909090910/0.9090909090909091`，trial2 `0.9148936170212766/0.9795454545454546/0.9000000000000000`；best by `val_acc` then `val_auc` 为 trial0，`peak_vram≈2.15 GiB`, `total_seconds≈779.9` → **discard**（best seed42 低于 matched equal-weight seed42 和 DFR-25 seed42；不能用 AUC spike 替代主指标。）
- **fusion-weight 结论**：best trial0 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0009_20260513_002819/trials/trial_0000/run/fusion_weight_analysis.json) 显示 label-evidence rank auxiliary 没有修复 routing。mean fusion weight `axial/coronal/sagittal = 0.8889762261446487/0.040047823349905615/0.07097594603955587`；top-weight count/rate `axial=94/1.0000`, `coronal=0/0.0000`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 86/4/4`，`top_weight_hit_rate.true_margin=0.9149`。即使 val set 仍有 8 个 non-axial top-true-margin 样本，gate top-weight 完全没有迁出 axial，说明这个 label-evidence mixture 主要强化了轴位主导，而不是学到可用的弱视角 rescue。
- **当前判断**：DFR-104 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它排除了“只用 detached true-label evidence CE 就能让 gate 学会 right-view routing”的假设；当前弱视角 evidence 对训练目标仍不足以改变 top routing，且主指标跌到 `0.9149`。下一轮不应继续扫 label-evidence rank aux weight；若继续主线，应把目标限定到 DFR-25 错误且 non-axial true-margin 明确的少数样本，或者先审计这些样本上 view logits / confidence logit 的可分性，避免再用全局 CE 进一步强化 axial。
...
## 2026-05-13：Decision-Fusion Repair Follow-up（DFR-107 confirmed-target rank-margin gate aux，main-study）

> **DFR-107 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-106 证明 targeted aux 的触发集合很干净，覆盖 `6/6` 个 DFR-25 错误和 `8/9` 个 non-axial true-margin 样本，但 DFR-105 的 pair-normalized soft CE 完全没有把 target view 推到 top/near-top；本轮只把训练信号改成更直接的 non-axial-vs-axial gate log-weight rank-margin target。
> - 如果成功，为什么有机会把 learned full-fusion `val_acc` 推到 matched `equal-weight` 之上？推理期仍是普通 learned decision fusion，不固定均权，也不切换 backbone / geometry / fusion family；预期不是把三视角平均化，而是让 DFR-106 确认的少量 coronal/sagittal target 从 DFR-105 的 target top/near-top `0/8` 迁到少数 top/near-top routes，同时全局 top-weight 仍保持 axial-majority（约 `88-92/94` axial）。如果这些迁移命中 DFR-25 错误或 non-axial true-margin 样本，就有机会把 learned full-fusion 的 seed42 `val_acc` 稳定推过 matched equal-weight，并尝试超过当前 DFR-25 seed42。

- [x] **DFR-107-RESNEXT-DECISION-256X8-CONFIRMED-TARGET-RANK-MARGIN-AUX-SEED42-MAIN-STUDY**：commit `b583d16`，在 [src/model.py](/dataset/HH/ankle-ct/src/model.py) 新增默认关闭的 `ANKLE_DECISION_ENABLE_GATE_CONFIRMED_TARGET_RANK_MARGIN_AUX_LOSS=1`，并增加 `ANKLE_DECISION_GATE_CONFIRMED_TARGET_RANK_MARGIN_AUX_WEIGHT/GAP/MARGIN`；aux 使用 detached per-view log-prob 找到 non-axial evidence 超过 axial 且 non-axial 自身类别 margin 非负的 confirmed target，然后把 `log w_target - log w_axial - margin` 编进现有 aux hook。fresh adaptive `main-study` 使用 [autoresearch_logs/generated_search_configs/optuna_main_search_iter_0002_20260513_023419.yaml](/dataset/HH/ankle-ct/autoresearch_logs/generated_search_configs/optuna_main_search_iter_0002_20260513_023419.yaml)，study root [runs/optuna_main_autoloop/iter_0002_20260513_023419](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0002_20260513_023419)，GPU policy `--gpu-ids 0,1,2 --max-workers 3`，固定 DFR-25 geometry/scalars，只跑一个离散变量 `ANKLE_DECISION_GATE_CONFIRMED_TARGET_RANK_MARGIN_AUX_MARGIN=0.10/0.25/0.50`。设计思路：沿 DFR-106 的 next-step audit path，不再扩大 target 或切换方向，而是把已确认的窄 target 监督从 soft pair CE 改成更强的 rank-margin gate target，直接要求目标 non-axial gate log-weight 跨过 axial。预计改进效果：target 样本应从 DFR-105 的 target top/near-top `0/8` 迁到至少少数 coronal/sagittal top/near-top；全局 routing 仍应 axial-majority，理想 top-weight 从 DFR-25/DFR-105 的 `94/0/0` 移到约 `88-92` 个 axial top 与少量 confirmed non-axial top，而不是把权重做成平均，从而让 learned full-fusion seed42 `val_acc` 超过 matched equal-weight `0.9255319148936170` 并挑战 DFR-25 seed42。实验实际结果：trial0 margin `0.10` 为 `0.9148936170212766/0.9790909090909091/0.9090909090909091`，trial1 margin `0.25` 为 `0.9361702127659575/0.9759090909090908/0.9302325581395349`，trial2 margin `0.50` 为 `0.9148936170212766/0.9790909090909091/0.9090909090909091`；best by `val_acc` then `val_auc` 为 trial1，`peak_vram≈2.15 GiB`, `total_seconds≈778.4` → **discard**（best seed42 虽高于 matched equal-weight seed42，但只追平 DFR-25 seed42 accuracy，AUC 与 F1 低于 DFR-25 `0.9786363636363636/0.9333333333333333`，且没有形成新的 routing repair。）
- **fusion-weight 结论**：best trial1 的 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/optuna_main_autoloop/iter_0002_20260513_023419/trials/trial_0001/run/fusion_weight_analysis.json) 显示 rank-margin aux 仍未越过 axial top-rank。mean fusion weight `axial/coronal/sagittal = 0.8184998349306432/0.11316878649782627/0.0683313782743`；top-weight count/rate `axial=94/1.0000`, `coronal=0/0.0000`, `sagittal=0/0.0000`；`top_true_margin` 分布为 `axial/coronal/sagittal = 84/6/4`，`top_pred_margin` 分布为 `86/0/8`，`top_weight_hit_rate.true_margin=0.8936170212765957`。也就是说，本轮确实把 mean non-axial mass 从 DFR-25 reference 的 `0.1114` 提到约 `0.1815`，但所有样本 top-weight 仍是 axial，未把 confirmed non-axial evidence 转成实际 top routing。
- **当前判断**：DFR-107 是 negative seed42 candidate study，不应 promotion 到 3-seed formal。它验证了“单纯把 DFR-105 的 target CE 加 hard rank-margin”仍不足以修复 routing：non-axial mean weight 上升，但 top-rank 完全不迁移，且指标只追平 DFR-25 accuracy、AUC 回落。下一轮不应继续扫 margin；更直接的下一步是做 DFR-107 target-sample 级审计，特别检查现有 aux hook 的 patient-label CE 编码是否对 label-0 normal rescue target 产生反向压力，然后再决定是否需要一个 label-invariant 的 confirmed-target gate objective 或等价的模型内 workaround。
...
### DFR-123 full metadata duplicate scan（2026-05-14）

> **DFR-123 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-121/122 已证明 remaining FN 与 duplicate validation evidence 相关；本轮只读扩大到 full metadata，确认是否还有跨 split 重复体数据，避免把数据泄漏/重复病例误解释成 learned gate 机制收益或失败。
> - 如果成功，为什么有机会把 learned/posthoc full-fusion `val_acc` 推到 matched `equal-weight` 之上？它不改模型也不改官方选择规则，但能决定当前 validation ceiling 和训练/验证边界是否可信；如果存在 train-val 重复，应先量化其影响，再判断 DFR-116 相对收益是否仍是模型/校准机制本身。

- [x] **DFR-123-RESNEXT-DECISION-256X8-FULL-METADATA-DUPLICATE-SCAN-ANALYSIS**：commit `acc0cc3`，新增 [scripts/report_dfr123_full_metadata_duplicate_scan.py](/dataset/HH/ankle-ct/scripts/report_dfr123_full_metadata_duplicate_scan.py)，只读扫描 `data/realdata/metadata.csv` 的 train/val/test NIfTI 路径；先按 resolved path 报告重复路径，再只对同文件大小的 NIfTI 候选组计算 SHA256（exact duplicate 不可能跨不同文件大小），完整报告写入 [autoresearch_logs/dfr123_full_metadata_duplicate_scan.json](/dataset/HH/ankle-ct/autoresearch_logs/dfr123_full_metadata_duplicate_scan.json)。设计思路：不读取预测、不读取 test 指标、不修改数据文件或 split，只报告 hash/group/split/label 分布。预计改进效果：若 DFR-121 duplicate 只是孤立 val-val 现象，full scan 应只发现该组；若存在 train-val 重复，则需要把 validation 指标解释改为包含 leakage/duplicate sensitivity。实验实际结果：metadata `376` 行，`376` 个唯一 patient_id，`376` 个唯一 NIfTI 路径，missing/invalid path `0`，duplicate patient_id `0`，multi-patient same-path group `0`；size prefilter 只需 hash `6` 个路径，发现 `3` 个 content duplicate groups / `6` 个 patient，其中 `2` 个是 train-val 阳性重复：`CTyang__CT24yang1__CT2412yang17`(val) = `CTyang__CT24yang2__CT2412yang13`(train)，`CTyang__CT24yang1__CT2412yang29`(val) = `CTyang__CT24yang2__CT2412yang19`(train)；另 `1` 个是 DFR-121 已知 val-val 阳性重复 `CTyang__CT24yang1__CT2412yang3` = `CTyang__CT24yang2__CT2412yang1`。所有重复均同 label，无 cross-label duplicate → **keep as data-integrity audit**。
- **当前判断**：DFR-123 把问题从单个 validation duplicate 扩展为 small but real metadata duplicate/leakage concern。下一轮仍不改数据文件；应做 DFR-124 read-only metric sensitivity，把 val-val duplicate 与 train-val duplicated validation patients 分开降权/剔除，比较 DFR-25 与 DFR-116 的 official full-validation mean、duplicate-clean sensitivity mean 与 DFR116-DFR25 delta，确认当前 posthoc branch 相对收益是否仍稳定。

### DFR-124 duplicate/leakage metric sensitivity（2026-05-14）

> **DFR-124 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-123 显示 validation 中既有 val-val duplicate FN，也有 train-val duplicate positive samples；本轮只读量化这些重复样本对 DFR-25/DFR-116 指标和相对 delta 的影响，避免继续用被重复结构扭曲的绝对指标误导 gate/posthoc 机制判断。
> - 如果成功，为什么有机会把 learned/posthoc full-fusion `val_acc` 推到 matched `equal-weight` 之上？它不改模型，但能确认 DFR-116 的相对收益是否在 duplicate/leakage-sensitive 评估下仍稳定；若 delta 保持，则说明当前 posthoc calibration branch 不是靠 train-val 重复或 val-val weighting 伪影得到。

- [x] **DFR-124-RESNEXT-DECISION-256X8-DUPLICATE-LEAKAGE-METRIC-SENSITIVITY-ANALYSIS**：commit `a446411`，新增 [scripts/report_dfr124_duplicate_leakage_metric_sensitivity.py](/dataset/HH/ankle-ct/scripts/report_dfr124_duplicate_leakage_metric_sensitivity.py)，读取 DFR-123 duplicate groups 与 DFR-25/DFR-116 validation telemetry，只读计算 full、val-val duplicate weighted/keep-first、train-val duplicated validation excluded、combined weighted/excluded 等诊断指标；完整报告写入 [autoresearch_logs/dfr124_duplicate_leakage_metric_sensitivity.json](/dataset/HH/ankle-ct/autoresearch_logs/dfr124_duplicate_leakage_metric_sensitivity.json)。设计思路：不修改 split，不读取 test 指标，不把这些指标作为官方模型选择，只判断 duplicate/leakage 对 current validation ceiling 与 DFR116-DFR25 delta 的影响。预计改进效果：val-val FN 去重后 absolute metric 应上升；剔除 train-val duplicated val patients 若它们都是 easy positives，absolute metric 可能小幅下降；关键是 DFR-116 相对 DFR-25 的 delta 应保持为正。实验实际结果：official full DFR-25 `0.939716/0.967727/0.935893` vs DFR-116 `0.950355/0.971515/0.946965`，delta `+0.010639/+0.003788/+0.011072`；val-val weighted DFR-25 `0.949821/0.977675/0.946612` vs DFR-116 `0.960573/0.981550/0.957850`，delta `+0.010752/+0.003875/+0.011238`；train-val val-patient excluded DFR-25 `0.938406/0.968572/0.932854` vs DFR-116 `0.949275/0.971905/0.944439`，delta `+0.010869/+0.003333/+0.011585`；combined clean sensitivity DFR-25 `0.948718/0.979024/0.944052` vs DFR-116 `0.959707/0.982439/0.955819`，delta `+0.010989/+0.003415/+0.011767`。The two train-val duplicated validation positives are correct under both branches in all three seeds; the val-val duplicate group is the repeated FN in all seeds → **keep as duplicate/leakage sensitivity report**。
- **当前判断**：DFR-124 confirms DFR-116 relative gain is robust to the discovered duplicate/leakage-sensitive validation policies, while absolute validation ceiling should be reported with duplicate caveats. 下一轮不应改数据文件；继续做 DFR-125 targeted clean-frontier false-case audit：在 combined clean sensitivity policy 下重新列出 DFR-116 remaining errors、protected fixed cases、以及是否还有 model-side no-harm posthoc candidate，避免把重复 FN 继续作为主要优化目标。

### DFR-125 combined-clean false-case/frontier audit（2026-05-14）

> **DFR-125 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-124 表明 DFR-116 相对收益在 clean policy 下仍稳定；本轮把 DFR-117 frontier 与 DFR-120 false-case report 改到 combined-clean 口径，确认去掉 train-val duplicate positives 与 val-val duplicate 副本后，remaining errors 是否还有安全 posthoc/gate 余量。
> - 如果成功，为什么有机会把 learned/posthoc full-fusion `val_acc` 推到 matched `equal-weight` 之上？如果 clean 口径下出现 improved no-harm candidate，就能在不利用重复验证样本的情况下继续扩展 DFR-116；如果没有，则说明下一步应停止阈值扩张，转向数据/采样/per-view evidence 审计。

- [x] **DFR-125-RESNEXT-DECISION-256X8-COMBINED-CLEAN-FRONTIER-FALSE-CASE-AUDIT-ANALYSIS**：commit `bb0864a`，新增 [scripts/report_dfr125_clean_frontier_false_case_audit.py](/dataset/HH/ankle-ct/scripts/report_dfr125_clean_frontier_false_case_audit.py)，使用 DFR-124 combined-clean policy（剔除 train-val duplicated validation patients、val-val duplicate keep-first）重算 DFR-25 clean、DFR-116 combo clean、remaining false cases 与 DFR-117 one-extra-posthoc candidate frontier；完整报告写入 [autoresearch_logs/dfr125_clean_frontier_false_case_audit.json](/dataset/HH/ankle-ct/autoresearch_logs/dfr125_clean_frontier_false_case_audit.json)。设计思路：不修改数据文件，不读取 test 指标，只在 clean validation mask 下复核 DFR-116 后是否还存在 safe expansion。预计改进效果：若此前 `14` 个 remaining errors 主要由 duplicate 结构驱动，clean 口径会减少 FN 计数；若仍有可泛化 posthoc 余量，`improved_no_harm_candidate_count` 应大于 0。实验实际结果：DFR-25 clean `0.948718/0.979024/0.944052`、top `229/0/44`；DFR-116 clean `0.959707/0.982439/0.955819`、top `228/0/45`、fixed `3`/broken `0`。remaining errors 从 `14` 降到 `11`（`7 FP + 4 FN`，`7` unique patients）：`CTyang...3` 仍为 3-seed FN，`CTyang...54` 为 single-seed FN；FP 仍是 strong/above-cap axial FP (`CTyin113/95/109/122/21`)。候选数 `3329`，no-harm `1560`，improved no-harm `0`；best no-harm 只把 AUC 提到 `0.986504` 但 fixed 仍 `3`，best fixed>combo 多修 `1` 个 FP 但新增 `2` 个错误，clean mean 降到 `0.956044/0.982276/0.951200` → **discard as no safe expansion**。
- **当前判断**：DFR-125 关闭了 clean 口径下继续扩 posthoc threshold 的路径。剩余 FN 是 duplicate-family true hard positive 与 no-view-weak evidence，剩余 FP 是 axial view classifier 强阳性误判，不能靠简单 gate residual 安全修复。下一轮 DFR-126 应做 targeted clean remaining evidence/sampling audit：只读检查 clean remaining 7 个 patient 的 NIfTI shape、baseline sampling覆盖、HU/STD peaks 与强 axial FP 的 per-view evidence，判断是否还存在可训练的 view-specific calibration/sampling 机制，而不是继续扫 posthoc 阈值。

### DFR-126 clean remaining evidence/sampling audit（2026-05-14）

> **DFR-126 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-125 已关闭 clean 口径下的安全 posthoc threshold 扩张；本轮只读检查剩余 clean false patients 的 per-view evidence 与 NIfTI sampling coverage，判断错误源到底是 gate 迁移不足、view classifier 本身强误判，还是采样覆盖不足。
> - 如果成功，为什么有机会把 learned/posthoc full-fusion `val_acc` 推到 matched `equal-weight` 之上？它不直接提升指标，但能把下一步从盲目 gate 阈值扩张收束到可检验机制：例如 strong axial FP 需要 view-specific calibration / axial risk modeling，而 weak positive FN 需要 sampling/annotation review 或更强 per-view evidence extraction。

- [x] **DFR-126-RESNEXT-DECISION-256X8-CLEAN-REMAINING-EVIDENCE-SAMPLING-AUDIT-ANALYSIS**：commit `8cf074c`，新增 [scripts/report_dfr126_clean_remaining_evidence_sampling.py](/dataset/HH/ankle-ct/scripts/report_dfr126_clean_remaining_evidence_sampling.py)，读取 DFR-125 clean remaining cases、DFR-116 telemetry、metadata 与对应 NIfTI，只读输出每个目标 patient 的 DFR-116 per-seed evidence、volume shape、baseline 8-slice/trim2 sampling coverage、HU/STD peak flags 与 duplicate annotations；完整报告写入 [autoresearch_logs/dfr126_clean_remaining_evidence_sampling.json](/dataset/HH/ankle-ct/autoresearch_logs/dfr126_clean_remaining_evidence_sampling.json)。设计思路：不修改数据文件、不读取 test 指标、不再搜索阈值，只把 clean remaining 7 个 patient 的错误机制分类。预计改进效果：如果剩余错误仍是 gate routing 问题，应出现 strong non-axial evidence 被 gate 压制；如果是 classifier/sampling 问题，则会表现为 axial 强 FP 或 positive 无 view 到达 weak abnormal。实验实际结果：7 个 patient / 11 cases 被分为 `negative_strong_axial_classifier_fp=5`、`positive_recurrent_subthreshold_evidence=1`、`positive_no_view_reaches_weak_abnormal=1`。5 个 negative FP 的 error-seed axial abnormal 均 `>=0.90`（例如 `CTyin95` axial `0.957-0.993`、`CTyin113` axial `0.959-0.972`），说明主要是 axial view classifier 强误判而非 gate threshold 不足；两个 positive FN 分别是 duplicate-family recurrent subthreshold (`CTyang...3`) 与 no-view-weak (`CTyang...54`)，并伴随 coronal sampling coverage flags。sampling flags 主要为 `coronal:low_hu300_peak_coverage=5`、`coronal:baseline_misses_std_top10=3`，另有 `sagittal:baseline_misses_hu300_top10=1` → **keep as mechanism audit**。
- **当前判断**：DFR-126 将 clean remaining errors 收束到两条机制：strong axial FP 需要 view-specific classifier calibration/risk audit，positive weak-evidence FN 更像 sampling/annotation/per-view sensitivity 问题。下一轮 DFR-127 应继续只读做 axial FP calibration frontier：在不改数据、不用 test 的前提下，基于 DFR-25/DFR-116 validation telemetry 比较 axial abnormal、non-axial normal evidence 与 clean FP/TN 的可分性，判断是否存在训练侧可表达的 axial-risk calibration signal；若没有，再考虑转向 sampling/augmentation 机制。
...
### DFR-132 global n32 sampling formal（2026-05-14）

> **DFR-132 自检**
> - 这项改动是否直接帮助 decision fusion 还原各视角应有作用？是。DFR-131 指出 clean remaining positive FN 的 coronal HU/STD coverage flags 在 baseline 8-slice/trim2 下更高，而 `n32_trim2` 是不改 `src/dataset.py` 时最强可表达 runtime sampling 候选。
> - 如果成功，为什么有机会把 learned/posthoc full-fusion `val_acc` 推到 matched `equal-weight` 之上？如果更多 slice 能让 coronal classifier 看到真实异常峰值，full fusion 可继续保持 DFR-25 的 learned decision-fusion 框架，但 coronal evidence/weight 不应退化；预期至少修复 DFR-126 weak-positive FN，且不扩大 strong axial FP。

- [x] **DFR-132-RESNEXT-DECISION-256X32-SAMPLING-FORMAL-SEED42**：commit `5f791cb`，新增 [configs/cmp_resnext_decision_256x32_dfr132_sampling_formal_s42.yaml](/dataset/HH/ankle-ct/configs/cmp_resnext_decision_256x32_dfr132_sampling_formal_s42.yaml) 与 [scripts/slurm_dfr132_sampling_256x32_formal.sbatch](/dataset/HH/ankle-ct/scripts/slurm_dfr132_sampling_256x32_formal.sbatch)，从 DFR-25 anchor 只改 `data.num_slices_per_view=32`，保持 `trim_edge_slices=2`、`image_size=256`、batch/scalars/runtime env 与 ResNeXt decision fusion 不变。Slurm job `486935` 在 node16 显式覆盖到空闲物理 GPU `4` 后完成，并生成 [fusion_weight_analysis.json](/dataset/HH/ankle-ct/runs/resnext_decision_256x8_mainline/dfr132_sampling_256x32_formal_s42/fusion_weight_analysis.json)。设计思路：用 DFR-131 最强 sampling candidate 检验全局更多 slice 是否能把 coronal 弱阳性证据带入训练，而不是继续 gate threshold/aux 修补。预计改进效果：coronal per-view sensitivity 或 positive FN evidence 应改善；mean coronal weight 不应塌缩，strong axial FP 不应扩大。
- **实验实际结果**：best validation `val_acc/auc/f1=0.8829787234042553/0.9518181818181818/0.8817204301075269`，显著低于 DFR-25 seed42 `0.9361702127659575/0.9786363636363636/0.9333333333333333` 与 DFR-116 seed42 `0.9574468085106383/0.9831818181818182/0.9545454545454546`；峰值显存约 `6.6GB`。Telemetry 显示 routing 反而更轴位锁定：mean weight `axial/coronal/sagittal = 0.9686954288 / 0.0126226406 / 0.0186819319`，top-weight `94/0/0`，top true-margin `78/3/13`；coronal 单视角 metrics 降到 `accuracy=0.5319148936170213`, `f1=0.0`, `auc=0.8336363636363636`。相对 DFR-25，DFR-132 只修复 `CTyin__CT24yin122` 与 `CTyin__CT24yin21`，但打坏 `7` 个病例（含 `CTyang__CT24yang1__CT2412yang17`, `CTyin__CT24yin109`, `CTyin__CT24yin116`, `CTyin__CT24yin125`, `CTyin__CT24yin17`, `CTyin__CT24yin182`, `CTyin__CT24yin39`）；相对 DFR-116 仅 fixed `1`、broken `8`，并失去 DFR-116 修复的 `CTyin__CT24yin95` → **discard**。
- **当前判断**：全局 `n32_trim2` 虽在静态 sampling audit 中覆盖异常峰值最多，但训练后没有转化为 coronal evidence，反而压低 coronal/sagittal 权重并扩大 axial FP；不要推进 `n32` multi-seed。DFR-131 第二候选 `n24_trim2` 与 n32 分数接近、无 clean-correct coronal collateral，而且可能比 n32 更少引入训练分布/证据稀释；下一轮 DFR-133 应跑 `num_slices_per_view=24, trim_edge_slices=2` 的同一 single-variable seed42 formal probe，若仍导致 coronal F1/weight 塌缩，则关闭全局 sampling-size family，转向不改 dataset 的训练/augmentation evidence calibration 或只读标注审计。
...
| 字段 | 值 |
|------|-----|
| 上次实验 | `FF-PAIRWISE-TOKEN-RESIDUAL-3SEED-MAIN`：在 feature-fusion gated-head anchor 的 CrossViewAttention mixer 后加入零初始化低容量 pairwise view-token residual。 |
| 上次结果 | commit `baa8347`，fresh study `runs/optuna_main_autoloop/iter_0002_20260528_013823` 完成 seeds `42/123/456`；结果 `0.925532/0.958182/0.919540`, `0.925532/0.956818/0.915663`, `0.914894/0.964545/0.906977`，3-seed mean `0.921986/0.959848/0.914060`，peak_vram max `2.18 GiB`。pairwise residual 权重非零学习（final adapter norms `0.229/0.124/0.281`），但低于 full gated-head anchor `fe42d90` mean `0.939716/0.975909/0.934415` → discard。 |
| 下一步 | `FF-PAIRWISE-FIXED-BROKEN-PATTERN-AUDIT`：基于 fixed/broken seed-case 列表继续做只读模式审计，寻找下一轮 feature-fusion narrow hypothesis；不使用测试集指标。 |
