...
| 字段 | 值 |
|------|-----|
| 上次实验 | MAIN-SYNC-01：将 `origin/main`（`f4e6d53`，含双 GPU 并行框架）合入训练分支；按“主分支代码优先、自动调参配置转 candidate”的规则保留训练线成果，并用 merged code 跑 1 次 proxy 验证 |
| 上次结果 | discard（集成验证通过：`val_acc=0.8404`, `val_auc=0.9182`, `val_f1=0.8193`, `peak_vram=14.4 GiB`, `total_seconds=618.4`；但低于训练线 pre-merge winner `a60c3e0` 的 `val_acc=0.8830`） |
| 下一步 | 只把已验证有效的内容择优并回 `main`：保留 official main 代码与正式配置，只并入 `configs/candidates/*`、实验记录与 main-sync 验证结论；若继续确认 trial 0 方向，优先跑 `configs/candidates/autoresearch_formal_trial0_20260413.yaml` |
| 连续 discard 计数 | 0（本次是分支集成验证，不计入调参 discard） |
| 累计 proxy keep 数 | 7（保留训练线 pre-merge winner `a60c3e0` 的 1 次新增 keep） |
...
|------|---:|-------------|------|------|
| **val_acc** | **0.8829787234042553** | `a60c3e0` | pre-merge training branch / fresh proxy study trial 0 | 训练线本地最佳；尚未在 merged main code 上复现 |
| **tie-break** | **0.9318181818181819** | `a60c3e0` | 与上述 winner 同一 trial | `val_acc` 持平时按此 `val_auc` 决胜 |
| main-sync validation | 0.8404255319148937 | `066ab57` | `configs/candidates/autoresearch_proxy_trial0_20260413.yaml` on merged main code | 集成验证通过，但不胜出 |
...
### 2026-04-13：pre-merge training-branch reference

- [x] **Baseline rerun**：当前 canonical baseline（未加结构改动）→ `val_acc=0.8404255319148937`, `val_auc=0.9181818181818182`, `val_f1=0.8192771084337349`, `peak_vram=14.4 GiB`。
- [x] **VRG-LOGIT-01 / winner**：pre-merge training branch 的 fresh proxy Optuna best completed trial（trial 0）→ `val_acc=0.8829787234042553`, `val_auc=0.9318181818181819`, `val_f1=0.8764044943820225` → **keep**。

### 2026-04-13：MAIN-SYNC-01 主分支同步 + candidate 验证

- [x] **Main merge**：将 `origin/main`（through `f4e6d53`）合入训练分支，双 GPU 并行框架与主分支训练代码全部保留；训练线的 `unsquashed reliability logits` 模型改动不直接带回主线。
- [x] **Candidate staging**：将训练线 trial 0 配置保留为 `configs/candidates/autoresearch_proxy_trial0_20260413.yaml` 和 `configs/candidates/autoresearch_formal_trial0_20260413.yaml`，不覆盖正式 `configs/autoresearch_proxy.yaml` / `configs/autoresearch_formal.yaml`。
- [x] **Proxy validation**：merged main code + proxy candidate 配置 → `val_acc=0.8404255319148937`, `val_auc=0.9181818181818182`, `val_f1=0.8192771084337349`, `peak_vram=14.4 GiB`, `total_seconds=618.4`。结论：**能正常工作**，但按当前指标体系对 pre-merge winner 记为 **discard**。
