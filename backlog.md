...
| 字段 | 值 |
|------|-----|
| 上次实验 | `DFR-57 view-role confidence multiseed formal`：从 DFR-25 fork，只把 gate confidence scorer 替换成 shared `ViewRoleConfidenceScorer`，使用 sample-centered view features + learned anatomical role embeddings，并把 confidence logits 限制在 `1.5`；formal seeds `42/123/456` 在 Slurm job `482374` 后续 node03 allocation 上并行完成，权重 telemetry 已补齐。 |
| 上次结果 | **discard with positive signal**。3-seed mean `val_acc=0.9290780141843971`, `val_auc=0.9730303030303031`, `val_f1=0.9224288966522939`，高于 matched equal-weight mean `0.9219858156028368 / 0.9606060606060606 / 0.9112221100424511`，但低于当前 DFR-25 mean `0.9397163120567376 / 0.9677272727272728 / 0.9358934169278997` 的主指标和 F1。fusion telemetry 显示 top-weight 不再 hard 单视图：seed42 `48/46/0`，seed123 `33/61/0`，seed456 `74/9/11`；role-aware scorer 有效反坍塌，但完全替换 DFR-25 scorer 会损失 seed42/456 ranking。 |
| 下一步 | 继续留在 DFR-25 decision-fusion collapse repair 主线，第三轮直接做 `DFR-58 DFR-25 base scorer + bounded view-role confidence blend`：保留原 confidence heads / calibrator 的 strong-evidence ranking，只把 DFR-57 的 role-aware scorer 作为小比例 logit blend 注入，目标是恢复 DFR-25 mean 同时减少 hard axial/single-plane collapse。 |
| 默认执行策略 | 24GB+ 显存机器默认直接跑 `main` / `formal`；`proxy` 仅保留作低显存 fallback 与快速 smoke。 |
| 连续 discard 计数 | 32（自 `DFR-26 seed123` 起至 `DFR-57 view-role confidence multiseed formal` 连续为 discard；上一轮 keep 已在 `DFR-26 seed42` 处把计数清零。虽然计数已超过 5，但用户已明确要求本次继续完成 3 轮 manual autoresearch，且 DFR-57 已给出直接服务于 `innovation.md` 和单视图独占修复的 positive routing signal，因此继续执行而不暂停询问。） |
