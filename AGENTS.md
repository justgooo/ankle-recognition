...
- GPU: 双卡环境（`nvidia-smi` 显示 `0=RTX 3090`, `1=RTX 4090`；但本机 PyTorch/CUDA 运行时顺序相反，实测 `cuda:0=RTX 4090`, `cuda:1=RTX 3090`）
- 设备编号注意：在当前机器上，`torch.device("cuda:1")` 实际会命中 **3090**；同理，`CUDA_VISIBLE_DEVICES=1` 暴露给训练进程的也是 **3090**。如果要命中 **4090**，应使用 `torch.device("cuda:0")` 或 `CUDA_VISIBLE_DEVICES=0`。不要把 `nvidia-smi` 的 index 直接当作 PyTorch 的 device id
