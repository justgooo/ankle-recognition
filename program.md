...
> [!IMPORTANT]
> **2026-05-26 人类主线重定向**
> - 当前 autoresearch 的唯一主方向改为：**特征融合（feature fusion）探索**。
> - 2026-04-23 / 2026-05-10 的 decision-fusion 锁定规则保留为历史记录，但不再约束当前主线；不得再因为旧规则阻止 feature-fusion 实验。
> - 当前 feature-fusion anchor 是 `PAPER-FEATURE-FUSION-GATED-HEAD-3SEED-FORMAL` / `CMP-FAIR-V100-RESNEXT-MAIN-GATED-HEAD` 系列：`ResNeXt + 256x8 + feature fusion + mean pooling + trim_edge_slices=2 + per-view recalibration + light cross-view mixer + GLU gated fusion head`。
> - 当前 3-seed formal reference：gated-head `val_acc/val_auc/val_f1 = 0.939716 / 0.975909 / 0.934415`；matched minimal concat-MLP feature-fusion baseline `0.914894 / 0.972121 / 0.906842`。
> - 后续主线实验必须服务于 feature-fusion 表征/融合头改进，优先做单变量结构假设与 matched baseline 对照；不要再把 DFR gate/routing 修复作为默认下一步。
> - 每轮行动前必须自检：这项改动是否直接增强 feature-level multi-view representation、cross-view interaction、fused-token gating、或 feature-fusion 的稳定性；如果不能明确回答，这轮改动就不应执行。
> - 每条实验记录都必须完整写下：`设计思路`、`预计改进效果`、`实验实际结果`。其中“预计改进效果”必须说明预期 feature-fusion 表征或融合头行为如何变化，以及为什么这种变化有机会提高 `val_acc`。

> [!IMPORTANT]
> **2026-05-10 decision-fusion 主线（历史冻结）**
> - 旧 operational mainline 曾固定为 **DFR-25 ResNeXt 256x8 learned decision fusion**。
> - 该系列已经完成 DFR-45 到 DFR-142 的 gate/routing、posthoc、sampling 与 review-gate 分析；当前作为历史 comparator 保留，不再是默认训练方向。
> - 如需回到 DFR 系列，必须由人类重新明确指定。

## 研究策略

**当前阶段**：feature-fusion 主线探索 + 可复现实验 workflow 对齐
...
- **feature fusion**：per-view feature recalibration + light cross-view mixer + GLU gated fusion head
- **mean slice pooling**：`use_attention_pooling=false`
- 固定 anchor scalar：`lr=1e-4`、`weight_decay=1e-4`、`dropout=0.25`、`gradient_clip_norm=1.0`、`freeze_layers=3`
...
- 当前 feature-fusion anchor 不需要 DFR runtime env；不要把 `ANKLE_DISABLE_FUSION_CROSS_VIEW_MIXER`、`ANKLE_DECISION_*` gate repair 环境变量默认注入主线实验
...
- 默认 Optuna search config 是 feature-fusion gated-head fixed anchor；后续 ResNeXt feature-fusion 改进必须以单变量 hypothesis 新开或修改搜索空间，不得默认宽搜
...
- formal / main 实验（15 epochs，当前 canonical ResNeXt 256x8 feature-fusion gated-head 路径）预计以实际 GPU 为准；历史 RTX A6000 / V100 记录约 12-18 分钟
- proxy 实验（4 epochs，当前 canonical ResNeXt 256x8 feature-fusion gated-head 路径）仅在显存不足或需要快速 smoke 时使用
- `batch_size=6` 是当前 feature-fusion 配置锚点
...
然后读取：
- `runs/autoresearch_formal_resnext_feature_gated_256x8/summary.json`
...
如果发现当前 feature-fusion gated-head 输出目录（如 `runs/autoresearch_formal_resnext_feature_gated_256x8/summary.json` 或 `runs/autoresearch_proxy_resnext_feature_gated_256x8/summary.json`）已经存在但 ledger 尚未同步，先补记 `results.tsv` 与 `backlog.md`，再开始下一轮实验。
...
6. 如果运行成功：
   - 读取 `runs/autoresearch_formal_resnext_feature_gated_256x8/summary.json`
   - 对 feature-fusion 实验，默认记录主指标、结构改动、峰值显存与必要的消融/telemetry；`fusion_weight_analysis.json` 只在本轮实验显式涉及 decision-fusion 权重时才要求生成
   - 将 `best_val.accuracy` 与当前保留的最佳结果比较
   - 只有当 val_acc 提升时，才保留这个提交
   - 如果 val_acc 持平，优先选择 `val_auc` 更高的版本
   - 如果两者都持平，优先选择更简单的代码
   - 否则回退这次实验
   - 更新实验记录时，必须同时写入 `设计思路`、`预计改进效果`、`实验实际结果`
...
> 当前执行口径以上文“feature-fusion 主线探索 + 可复现实验 workflow 对齐”为准。
...
4. 如果 feature-fusion 结构方向连续失败，先做 matched 消融或 feature telemetry；不要默认切回 Decision Fusion，除非人类重新指定
5. 回顾基础超参是否合理
