...
The current goal is not merely to improve the learned branch against itself, but to restore the proper role of each view's information inside decision fusion, so the view that should lead can lead, the view that should supplement can supplement, and true multi-view full-fusion learned accuracy can exceed the matched `equal-weight` control before the mainline can move elsewhere.
...
2. Run `L2-minimal`, `L3-no-mixer`, and `L4-no-calibrator` as single-module ablations against `L1`, only if they have a direct mechanism link to restoring the proper role of each view's information inside full fusion
...
4. After `L5` closes, stop opening new `temp=*` ablations unless a later mechanism result forces a targeted re-check tied to restoring the proper role of each view's information
